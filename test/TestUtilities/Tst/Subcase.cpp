...
{
    const za::SizeT maxDepth = decisionPath.size() < discoveredDecisionPath.size() ? decisionPath.size()
                                                                                   : discoveredDecisionPath.size();

    for (za::SizeT depth = maxDepth; depth > 0u; --depth)
    {
        const za::SizeT index = depth - 1u;
...
{
    const za::SizeT depth = s.decisionDepth;
    ...
    if (s.discoveredDecisionPath.size() == depth)
    {
        ...
        if (s.decisionPath.size() == depth)
            s.decisionPath.pushBack(static_cast<za::SizeT>(0u));
    }
...
bool TraversalState::tryEnterSubcase(const SubcaseSignature& sig)
{
    ...
    za::SizeT       siblingIndex = 0u;
    const za::SizeT subcasesSize = subcases.size();
