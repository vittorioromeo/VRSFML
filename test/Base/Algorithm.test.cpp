...
#include "ZancleBase/Algorithm/Rotate.hpp"
...
TEST_CASE("[Base] Base/Algorithm/*.hpp")
{
    SECTION("Copy")
    {
        ...
        CHECK(zb::copy(values, values + 4, target) == target + 4);
    ...
    SECTION("Find/FindIf/AnyOf")
    {
        ...
        CHECK(zb::find(values, values + 8, 4) == &values[4]);
        CHECK(zb::find(values, values + 8, 5) == &values[5]);
        CHECK(zb::find(values, values + 8, 400) == values + 8);

        CHECK(zb::findIf(values, values + 8, [](const int x) { return x == 4; }) == &values[4]);
        CHECK(zb::findIf(values, values + 8, [](const int x) { return x == 5; }) == &values[5]);
        CHECK(zb::findIf(values, values + 8, [](const int x) { return x == 400; }) == values + 8);

        CHECK(zb::anyOf(values, values + 8, [](const int x) { return x == 4; }));
        CHECK(zb::anyOf(values, values + 8, [](const int x) { return x == 5; }));
        CHECK(!zb::anyOf(values, values + 8, [](const int x) { return x == 400; }));
    }

    SECTION("Count")
    {
        const bool bools[]{true, false, true, true, false};
        CHECK(zb::count(bools, bools + 5, true) == 3);

        const int ints[]{1, 0, 5, 0, -1, 0, 7};
        CHECK(zb::count(ints, ints + 7, 0) == 3);
    }
    ...
    SECTION("CountIf")
    {
        ...
        CHECK(zb::countIf(values, values + 8, isEven) == 4);
        CHECK(zb::countIf(values, values + 8, isGt5) == 2); // 6, 7
        CHECK(zb::countIf(values, values + 8, alwaysFalse) == 0);

        CHECK(zb::countIf(values, values + 0, isEven) == 0);
    }
    ...
    SECTION("AllOf")
    {
        ...
        CHECK(zb::allOf(values, values + 8, isLt10));
        CHECK(!zb::allOf(values, values + 8, isEven));

        const int evenValues[]{2, 4, 6, 8};
        CHECK(zb::allOf(evenValues, evenValues + 4, isEven));

        CHECK(zb::allOf(evenValues, evenValues + 0, isEven));     // Vacuously true
        CHECK(zb::allOf(evenValues, evenValues + 0, alwaysTrue)); // Vacuously true
    }
    ...
    SECTION("RemoveIf")
    {
        ...
        auto* newEnd = zb::removeIf(v.begin(), v.end(), isEven);
    ...
    SECTION("VectorEraseIf")
    {
        ...
        CHECK(v.size() == 4);
        CHECK((v == zb::Vector<int>{1, 3, 5, 7}));

        // Check with no elements removed
        removedCount = zb::vectorEraseIf(v, isEven);
        ...
        removedCount = zb::vectorEraseIf(v, isOdd);
        CHECK(removedCount == 4);
        CHECK(v.empty());
    ...
    SECTION("IsSorted")
    {
        ...
        CHECK(zb::isSorted(sorted, sorted + 6, less));

        const int unsorted[] = {1, 3, 2, 4, 5};
        CHECK(!zb::isSorted(unsorted, unsorted + 5, less));

        const int reverseSorted[] = {5, 4, 3, 2, 1};
        CHECK(!zb::isSorted(reverseSorted, reverseSorted + 5, less));
        CHECK(zb::isSorted(reverseSorted, reverseSorted + 5, greater)); // Check with different comparer

        const int single[] = {10};
        CHECK(zb::isSorted(single, single + 1, less)); // Single element is sorted

        CHECK(zb::isSorted(unsorted, unsorted + 0, less)); // Empty range is sorted
    }
    ...
    SECTION("VectorSwapAndPopIf")
    {
        ...
        CHECK(removedCount == 4);
        CHECK(v.size() == 4);
        CHECK((v == zb::Vector<int>{1, 5, 3, 7}));

        // Check with no elements removed
        removedCount = zb::vectorSwapAndPopIf(v, isEven);
        CHECK(removedCount == 0);
        CHECK(v.size() == 4);
        CHECK((v == zb::Vector<int>{1, 5, 3, 7}));
        ...
        // Check removing all elements
        auto isOdd   = [](int x) { return x % 2 != 0; };
        removedCount = zb::vectorSwapAndPopIf(v, isOdd);
        CHECK(removedCount == 4);
        CHECK(v.empty());
    ...
    SECTION("AdjacentFind (Default): Core functionality")
    {
        ...
        {
            int   vec[] = {5, 5, 2, 3, 4};
            auto* it    = zb::adjacentFind(vec, vec + 5);
            REQUIRE(it == vec);
            CHECK(*it == 5);
        }
        ...
        {
            int   vec[] = {1, 2, 2, 3, 4, 4};
            auto* it    = zb::adjacentFind(vec, vec + 6);
            REQUIRE(it == vec + 1);
            CHECK(*it == 2);
        }
    }
    ...
    SECTION("AdjacentFind (Default): Edge cases")
    {
        SUBCASE("No adjacent pair found")
        {
            ...
            auto* it    = zb::adjacentFind(vec, vec + 9);
            CHECK(it == vec + 9);
        }
        ...
        {
            int   vec[1]{};
            auto* it = zb::adjacentFind(vec, vec + 0);
            CHECK(it == vec + 0);
        }
        ...
        {
            int   vec[] = {100};
            auto* it    = zb::adjacentFind(vec, vec + 1);
            CHECK(it == vec + 1);
        }
    }
    ...
    SECTION("Adjacent find")
    {
        SUBCASE("Find where second element is greater than first")
        {
            ...
            auto* it = zb::adjacentFind(vec, vec + 5, greaterCmp);
    ...
    SECTION("Rotate")
    {
        SUBCASE("Rotate by 3 in middle of range")
        {
            ...
            auto* it = zb::rotate(values, values + 3, values + 7);
        ...
        SUBCASE("Rotate by 1")
        {
            ...
            auto* it = zb::rotate(values, values + 1, values + 4);
        ...
        SUBCASE("Rotate by size - 1 (single element to the back)")
        {
            ...
            auto* it = zb::rotate(values, values + 3, values + 4);
        ...
        SUBCASE("Rotate with middle == first is a no-op")
        {
            ...
            auto* it = zb::rotate(values, values, values + 4);
        ...
        SUBCASE("Rotate with middle == last is a no-op")
        {
            ...
            auto* it = zb::rotate(values, values + 4, values + 4);
        ...
        SUBCASE("Rotate at exact midpoint of even-sized range")
        {
            ...
            auto* it = zb::rotate(values, values + 3, values + 6);
        ...
        SUBCASE("Rotate with second segment larger than first (uneven)")
        {
            ...
            auto* it = zb::rotate(values, values + 2, values + 9);
