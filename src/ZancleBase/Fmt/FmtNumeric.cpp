...
FmtResult fmtArg(FmtSink& sink, const T& arg, const FmtSpec& spec)
{
    ...
    {
        ZB_ASSERT(false && "invalid integral format spec");
    ...
    if constexpr (ZB_IS_SAME(T, bool))
    {
        if (spec.type != '\0') [[unlikely]]
        {
            ZB_ASSERT(false && "invalid bool format spec");
            return FmtResult::Failed;
        }
    ...
    ZB_ASSERT(end != nullptr && "Numeric `fmtArg` ran out of buffer -- buf[64] should always fit");
...
FmtResult fmtArg(FmtSink& sink, const T& arg, const FmtSpec& spec)
{
    ...
    {
        ZB_ASSERT(false && "invalid floating-point format spec");
        return FmtResult::Failed;
    }
...
ZB_FMT_INSTANTIATE(bool);

ZB_FMT_INSTANTIATE(signed char);
ZB_FMT_INSTANTIATE(short);
ZB_FMT_INSTANTIATE(int);
ZB_FMT_INSTANTIATE(long);
ZB_FMT_INSTANTIATE(long long);

ZB_FMT_INSTANTIATE(unsigned char);
ZB_FMT_INSTANTIATE(unsigned short);
ZB_FMT_INSTANTIATE(unsigned int);
ZB_FMT_INSTANTIATE(unsigned long);
ZB_FMT_INSTANTIATE(unsigned long long);

ZB_FMT_INSTANTIATE(float);
ZB_FMT_INSTANTIATE(double);
ZB_FMT_INSTANTIATE(long double);

#undef ZB_FMT_INSTANTIATE
...
FmtResult fmtArg(FmtSink& sink, const char& arg, const FmtSpec& spec)
{
    ...
    {
        ZB_ASSERT(false && "invalid char format spec");
        return FmtResult::Failed;
    }
