    ...
    const zb::SizeT   rhsSize)
{
    ...
    result.append(zb::StringView{lhs, lhsSize});
    result.append(zb::StringView{rhs, rhsSize});
...
void String::grow(const SizeT minCapacity)
{
    ...
    ZB_MEMCPY(newData, data(), currentSize);
...
{
    ZB_ASSERT(cStr != nullptr);

    if (count <= maxSsoSize)
    {
        ZB_MEMCPY(m_rep.sso.buffer, cStr, count);
        setSizeAndTerminate(count);
    }
    else
    {
        char* const newData = priv::VectorUtils::allocate<char>(count + 1);
        ZB_MEMCPY(newData, cStr, count);
...
////////////////////////////////////////////////////////////
String::String(const char* const cStr) : String{cStr, ZB_STRLEN(cStr)}
{
    ZB_ASSERT(cStr != nullptr);
}
...
{
    ZB_MEMCPY(&m_rep, &other.m_rep, sizeof(m_rep));
...
String& String::operator=(const StringView view)
{
    ...
    if (srcInsideThis)
    {
        ...
        if (newSize <= capacity())
        {
            ZB_MEMMOVE(data(), data() + offset, newSize);
        ...
        ZB_MEMCPY(newData, myData + offset, newSize);
        newData[newSize] = '\0';
    ...
    {
        ZB_MEMCPY(m_rep.heap.data, view.data(), view.size());
...
{
    ZB_ASSERT(!empty() && "popBack on empty string");
...
String& String::append(const StringView view)
{
    ...
    if (srcInsideThis)
        ZB_MEMMOVE(newMyData + currentSize, newMyData + srcOffset, otherSize);
    else
        ZB_MEMCPY(newMyData + currentSize, src, otherSize);
...
{
    const SizeT currentSize = size();
    ZB_ASSERT(index <= currentSize && "Index is out of bounds");
    ...
    ZB_MEMMOVE(data() + index, data() + index + count, tailLength);
...
{
    ZB_ASSERT(pos <= size() && "Insertion position is out of bounds");
    ...
    if (pos < oldSize)
        ZB_MEMMOVE(d + pos + 1, d + pos, oldSize - pos);
...
void String::insert(const SizeT pos, const char* const cStr)
{
    ...
    const SizeT insertCount = ZB_STRLEN(cStr);
    if (insertCount == 0u)
        return;
    ...
    ZB_MEMCPY(d + pos, cStr, insertCount);
...
{
    const SizeT oldSize = size();
    ZB_ASSERT(pos <= oldSize && "Replacement position is out of bounds");
    ...
    if (replSize != count && tailLength > 0u)
        ZB_MEMMOVE(d + pos + replSize, d + pos + count, tailLength);

    // Copy the replacement into the freshly-sized hole.
    if (replSize > 0u)
        ZB_MEMCPY(d + pos, replacement.data(), replSize);
...
void swap(String& lhs, String& rhs) noexcept
{
    ...
    ZB_MEMCPY(&temp, &lhs.m_rep, sizeof(String::RepUnion));
    ZB_MEMCPY(&lhs.m_rep, &rhs.m_rep, sizeof(String::RepUnion));
    ZB_MEMCPY(&rhs.m_rep, &temp, sizeof(String::RepUnion));
...
{
    ZB_ASSERT(lhs != nullptr);
    return operatorPlusImpl(lhs, ZB_STRLEN(lhs), rhs.data(), rhs.size());
}
...
{
    ZB_ASSERT(rhs != nullptr);
    return operatorPlusImpl(lhs.data(), lhs.size(), rhs, ZB_STRLEN(rhs));
}
