...
[[nodiscard]] sf::base::String tlsErrorString(int errnum)
{
    char buffer[1024]{};
...
// We just provide wrapper functions around `sf::AtomicMutex`
...
struct MbedTlsThreading
{
    ...
    MbedTlsThreading()
    {
        ...
        {
            if (ptr)
                *ptr = new sf::AtomicMutex;
        },
            [](mbedtls_threading_mutex_t* ptr) // Destruction
        {
            if (ptr)
                delete static_cast<sf::AtomicMutex*>(*ptr);
        },
            [](mbedtls_threading_mutex_t* ptr) // Lock
        {
            ...
            static_cast<sf::AtomicMutex*>(*ptr)->lock();
            return 0;
        },
    ...
    #endif
            static_cast<sf::AtomicMutex*>(*ptr)->unlock();
    #ifdef _MSC_VER
        #pragma warning(pop)
    #endif
            return 0;
...
struct MbedTlsSharedState
{
    ...
    #define SFML_NUMBER_TO_STRING(x)        SFML_NUMBER_TO_STRING_HELPER(x)
        ...
        static constexpr sf::base::StringView
...
TcpSocket::TlsStatus TcpSocket::setupTlsClient(const Utf8String& hostname, const char* certificateChainData)
{
    ...
    return setupTlsClient(hostname,
                          ...
                          SFML_BASE_STRLEN(certificateChainData) + 1);
}
...
{
    SFML_BASE_ASSERT(certificateChainData != nullptr && certificateChainSize > 0 && "Certificate chain data not valid");
    SFML_BASE_ASSERT(privateKeyData != nullptr && privateKeySize > 0 && "Private key data not valid");
...
Socket::Status TcpSocket::send(const void* data, base::SizeT size, base::SizeT& sent)
{
    ...
    for (sent = 0; sent < size; sent += static_cast<base::SizeT>(result))
    {
        if (m_impl->tlsState.hasValue())
        {
            ...
            SFML_BASE_ASSERT(m_impl->tlsState->handshakeComplete &&
