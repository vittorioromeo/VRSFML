...
std::uint32_t factorToGlConstant(sf::BlendMode::Factor blendFactor)
{
    ...
    sf::err() << "Invalid value for sf::BlendMode::Factor! Fallback to sf::BlendMode::Factor::Zero." << sf::errEndl;
    assert(false);
    return GL_ZERO;
}
...
std::uint32_t stencilOperationToGlConstant(sf::StencilUpdateOperation operation)
{
    ...
    sf::err() << "Invalid value for sf::StencilUpdateOperation! Fallback to sf::StencilMode::Keep." << sf::errEndl;
    assert(false);
    return GL_KEEP;
}
...
std::uint32_t stencilFunctionToGlConstant(sf::StencilComparison comparison)
{
    ...
    sf::err() << "Invalid value for sf::StencilComparison! Fallback to sf::StencilMode::Always." << sf::errEndl;
    assert(false);
    return GL_ALWAYS;
}
} // namespace RenderTargetImpl
    ...
    if (RenderTargetImpl::isActive(m_id) || setActive(true))
    {
        ...
        if (error != GL_NO_ERROR)
        {
            err() << "OpenGL error (" << error << ") detected in user code, "
                  << "you should check for errors with glGetError()" << errEndl;
        }
#endif
