...
Font::Font(base::PassKey<Font>&&, FontFace&& fontFace, TextureAtlas* textureAtlas) :
    m_impl{base::makeUnique<Impl>(SFML_BASE_MOVE(fontFace), textureAtlas)}
{
...
base::Optional<Font> Font::openFromFile(const Path& filename, TextureAtlas* textureAtlas)
{
    ...
    return base::makeOptional<Font>(base::PassKey<Font>{}, SFML_BASE_MOVE(*optFontFace), textureAtlas);
}
...
const Glyph& Font::getGlyph(const char32_t     codePoint,
                            ...
                            const float        outlineThickness) const
{
    SFML_BASE_ASSERT(m_impl->fontFace.hasGlyph(codePoint));
