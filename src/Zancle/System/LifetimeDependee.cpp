...
{
    return *ZB_LAUNDER_CAST(AtomicUInt*, ptr);
}
...
bool LifetimeDependee::TestingModeGuard::fatalErrorTriggered(const char* const dependeeName)
{
    return lifetimeTrackingFatalErrorTriggered.loadSeqCst() &&
           ZB_STRCMP(lifetimeTrackingTestingDependeeName.loadSeqCst(), dependeeName) == 0;
}
...
{
    ZB_PLACEMENT_NEW(m_dependantCount) AtomicUInt(0u);
}
...
{
    ZB_ASSERT(asAtomicUInt(m_dependantCount).loadRelaxed() > 0u);
