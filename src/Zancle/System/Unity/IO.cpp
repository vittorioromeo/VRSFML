...
[[maybe_unused, gnu::always_inline]] inline std::string toUtf8FilenameForStdio(zb::StringView v)
{
...
{
    ZB_ASSERT(!isAppend || ZB_IS_SAME(T, zb::Vector<char>));
    ...
    ZB_SCOPE_GUARD({ std::fclose(file); });
    ...
    const auto size = static_cast<zb::SizeT>(rawSize);
...
[[nodiscard]] bool nativeStatHandle(::HANDLE handle, zb::SizeT& outSize)
{
    ...
    if (static_cast<ULL>(fileSize.QuadPart) > static_cast<ULL>(static_cast<zb::SizeT>(-1)))
    {
        ::CloseHandle(handle);
        return false;
    }

    outSize = static_cast<zb::SizeT>(fileSize.QuadPart);
    return true;
}
...
[[nodiscard]] bool nativeOpenAndStat(zb::StringView filename, ::HANDLE& outHandle, zb::SizeT& outSize)
{
    ...
    widePath.resize_and_overwrite(static_cast<zb::SizeT>(wideLen),
                                  [&](wchar_t* buf, zb::SizeT n) -> zb::SizeT
    {
        return static_cast<zb::SizeT>(
...
[[nodiscard]] bool nativeOpenAndStat(const za::Path& filename, ::HANDLE& outHandle, zb::SizeT& outSize)
{
...
////////////////////////////////////////////////////////////
[[nodiscard]] bool nativeReadFully(::HANDLE handle, char* buffer, zb::SizeT size)
{
    while (size > 0u)
    {
        ...
        buffer += bytesRead;
        size -= static_cast<zb::SizeT>(bytesRead);
    }
    return true;
}
...
[[nodiscard]] bool nativeStatFd(int fd, zb::SizeT& outSize)
{
    struct ::stat st{};
    ...
    using ULL = unsigned long long;
    if (static_cast<ULL>(st.st_size) > static_cast<ULL>(static_cast<zb::SizeT>(-1)))
    {
        ::close(fd);
        return false;
    }

    outSize = static_cast<zb::SizeT>(st.st_size);
    return true;
}
...
[[nodiscard]] bool nativeOpenAndStat(zb::StringView filename, int& outFd, zb::SizeT& outSize)
{
...
[[nodiscard]] bool nativeOpenAndStat(const za::Path& filename, int& outFd, zb::SizeT& outSize)
{
...
////////////////////////////////////////////////////////////
[[nodiscard]] bool nativeReadFully(int fd, char* buffer, zb::SizeT size)
{
    while (size > 0u)
    {
...
template <typename T>
void dispatchReadFileContentsIntoBufferImpl(T& target, const zb::SizeT size, const bool isAppend, auto&& fFillBuffer)
{
    if constexpr (ZB_IS_SAME(T, std::string))
    {
    ...
    {
        static_assert(ZB_IS_SAME(T, zb::Vector<char>));

        const zb::SizeT origin    = isAppend ? target.size() : 0u;
        const zb::SizeT finalSize = origin + size;
...
////////////////////////////////////////////////////////////
// `Filename` is either `zb::StringView` or `const za::Path&`; both
...
bool readFromFileImpl(const Filename& filename, T& target, const bool isAppend)
{
    ...
    ZB_SCOPE_GUARD({ nativeClose(handle); });
    ...
    // `resizeAndOverwrite` on `zb::String` skip the zero-init pass that
