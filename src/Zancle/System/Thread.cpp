...
[[nodiscard, gnu::always_inline]] inline NativeHandle& asNative(void* const p) noexcept
{
    ...
    // construct the `NativeHandle` in place. `ZB_LAUNDER_CAST` tells
    ...
    return *ZB_LAUNDER_CAST(NativeHandle*, p);
}
...
{
    ZB_MEMCPY(dst.m_native, other.m_native, sizeof(dst.m_native));
    dst.m_id       = other.m_id;
    dst.m_joinable = other.m_joinable;

    ZB_MEMSET(other.m_native, 0, sizeof(other.m_native));
    other.m_id       = 0u;
