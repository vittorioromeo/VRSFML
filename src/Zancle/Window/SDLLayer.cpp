// LICENSE AND COPYRIGHT (C) INFORMATION
// https://github.com/vittorioromeo/VRSFML/blob/master/license.md


////////////////////////////////////////////////////////////
// Headers
////////////////////////////////////////////////////////////
#include "Zancle/Window/SDLLayer.hpp"

#include "Zancle/Window/Cursor.hpp"
#include "Zancle/Window/DisplayOrientation.hpp"
#include "Zancle/Window/Keyboard.hpp"
#include "Zancle/Window/Mouse.hpp"
#include "Zancle/Window/VideoMode.hpp"
#include "Zancle/Window/WindowHandle.hpp"
#include "Zancle/Window/WindowSettings.hpp"

#include "Zancle/System/Err.hpp"
#include "Zancle/System/Priv/Vec2Base.hpp"
#include "Zancle/System/Rect2.hpp"
#include "Zancle/System/Utf8String.hpp"

#include "ZancleBase/Abort.hpp"
#include "ZancleBase/Assert.hpp"
#include "ZancleBase/Builtin/Memcmp.hpp"
#include "ZancleBase/Builtin/Strcmp.hpp"
#include "ZancleBase/Builtin/Strlen.hpp"
#include "ZancleBase/IntTypes.hpp"
#include "ZancleBase/ScopeGuard.hpp"
#include "ZancleBase/SizeT.hpp"

#include <SDL3/SDL_clipboard.h>
#include <SDL3/SDL_error.h>
#include <SDL3/SDL_guid.h>
#include <SDL3/SDL_hints.h>
#include <SDL3/SDL_init.h>
#include <SDL3/SDL_joystick.h>
#include <SDL3/SDL_keyboard.h>
#include <SDL3/SDL_keycode.h>
#include <SDL3/SDL_mouse.h>
#include <SDL3/SDL_oldnames.h>
#include <SDL3/SDL_pixels.h>
#include <SDL3/SDL_properties.h>
#include <SDL3/SDL_rect.h>
#include <SDL3/SDL_scancode.h>
#include <SDL3/SDL_stdinc.h>
#include <SDL3/SDL_surface.h>
#include <SDL3/SDL_touch.h>
#include <SDL3/SDL_video.h>


////////////////////////////////////////////////////////////
#define ZA_PRIV_SDL_ATTR_X_LIST                                             \
    X(SDL_GL_RED_SIZE, "SDL_GL_RED_SIZE")                                     \
    X(SDL_GL_GREEN_SIZE, "SDL_GL_GREEN_SIZE")                                 \
    X(SDL_GL_BLUE_SIZE, "SDL_GL_BLUE_SIZE")                                   \
    X(SDL_GL_ALPHA_SIZE, "SDL_GL_ALPHA_SIZE")                                 \
    X(SDL_GL_BUFFER_SIZE, "SDL_GL_BUFFER_SIZE")                               \
    X(SDL_GL_DOUBLEBUFFER, "SDL_GL_DOUBLEBUFFER")                             \
    X(SDL_GL_DEPTH_SIZE, "SDL_GL_DEPTH_SIZE")                                 \
    X(SDL_GL_STENCIL_SIZE, "SDL_GL_STENCIL_SIZE")                             \
    X(SDL_GL_ACCUM_RED_SIZE, "SDL_GL_ACCUM_RED_SIZE")                         \
    X(SDL_GL_ACCUM_GREEN_SIZE, "SDL_GL_ACCUM_GREEN_SIZE")                     \
    X(SDL_GL_ACCUM_BLUE_SIZE, "SDL_GL_ACCUM_BLUE_SIZE")                       \
    X(SDL_GL_ACCUM_ALPHA_SIZE, "SDL_GL_ACCUM_ALPHA_SIZE")                     \
    X(SDL_GL_STEREO, "SDL_GL_STEREO")                                         \
    X(SDL_GL_MULTISAMPLEBUFFERS, "SDL_GL_MULTISAMPLEBUFFERS")                 \
    X(SDL_GL_MULTISAMPLESAMPLES, "SDL_GL_MULTISAMPLESAMPLES")                 \
    X(SDL_GL_ACCELERATED_VISUAL, "SDL_GL_ACCELERATED_VISUAL")                 \
    X(SDL_GL_RETAINED_BACKING, "SDL_GL_RETAINED_BACKING")                     \
    X(SDL_GL_CONTEXT_MAJOR_VERSION, "SDL_GL_CONTEXT_MAJOR_VERSION")           \
    X(SDL_GL_CONTEXT_MINOR_VERSION, "SDL_GL_CONTEXT_MINOR_VERSION")           \
    X(SDL_GL_CONTEXT_FLAGS, "SDL_GL_CONTEXT_FLAGS")                           \
    X(SDL_GL_CONTEXT_PROFILE_MASK, "SDL_GL_CONTEXT_PROFILE_MASK")             \
    X(SDL_GL_SHARE_WITH_CURRENT_CONTEXT, "SDL_GL_SHARE_WITH_CURRENT_CONTEXT") \
    X(SDL_GL_FRAMEBUFFER_SRGB_CAPABLE, "SDL_GL_FRAMEBUFFER_SRGB_CAPABLE")     \
    X(SDL_GL_CONTEXT_RELEASE_BEHAVIOR, "SDL_GL_CONTEXT_RELEASE_BEHAVIOR")     \
    X(SDL_GL_CONTEXT_RESET_NOTIFICATION, "SDL_GL_CONTEXT_RESET_NOTIFICATION") \
    X(SDL_GL_CONTEXT_NO_ERROR, "SDL_GL_CONTEXT_NO_ERROR")                     \
    X(SDL_GL_FLOATBUFFERS, "SDL_GL_FLOATBUFFERS")                             \
    X(SDL_GL_EGL_PLATFORM, "SDL_GL_EGL_PLATFORM")

namespace
{
////////////////////////////////////////////////////////////
[[nodiscard, gnu::const]] constexpr const char* getSDLGLAttrName(const SDL_GLAttr attribute) noexcept
{
    switch (attribute)
    {
#define X(attr, name) \
    case attr:        \
        return name;
        ZA_PRIV_SDL_ATTR_X_LIST
#undef X
    }

    return "unknown SDL GL attribute";
}

} // namespace


////////////////////////////////////////////////////////////
#undef ZA_PRIV_SDL_ATTR_X_LIST


////////////////////////////////////////////////////////////
#define ZA_PRIV_SFML_SDL_KEYCODE_MAPPING                                  \
    X(SDLK_UNKNOWN, ::za::Keyboard::Key::Unknown)                           \
    X(SDLK_RETURN, ::za::Keyboard::Key::Enter)                              \
    X(SDLK_ESCAPE, ::za::Keyboard::Key::Escape)                             \
    X(SDLK_BACKSPACE, ::za::Keyboard::Key::Backspace)                       \
    X(SDLK_TAB, ::za::Keyboard::Key::Tab)                                   \
    X(SDLK_SPACE, ::za::Keyboard::Key::Space)                               \
    X(SDLK_EXCLAIM, ::za::Keyboard::Key::Exclaim)                           \
    X(SDLK_DBLAPOSTROPHE, ::za::Keyboard::Key::DoubleApostrophe)            \
    X(SDLK_HASH, ::za::Keyboard::Key::Hash)                                 \
    X(SDLK_DOLLAR, ::za::Keyboard::Key::Dollar)                             \
    X(SDLK_PERCENT, ::za::Keyboard::Key::Percent)                           \
    X(SDLK_AMPERSAND, ::za::Keyboard::Key::Ampersand)                       \
    X(SDLK_APOSTROPHE, ::za::Keyboard::Key::Apostrophe)                     \
    X(SDLK_LEFTPAREN, ::za::Keyboard::Key::LParenthesis)                    \
    X(SDLK_RIGHTPAREN, ::za::Keyboard::Key::RParenthesis)                   \
    X(SDLK_ASTERISK, ::za::Keyboard::Key::Asterisk)                         \
    X(SDLK_PLUS, ::za::Keyboard::Key::Plus)                                 \
    X(SDLK_COMMA, ::za::Keyboard::Key::Comma)                               \
    X(SDLK_MINUS, ::za::Keyboard::Key::Hyphen)                              \
    X(SDLK_PERIOD, ::za::Keyboard::Key::Period)                             \
    X(SDLK_SLASH, ::za::Keyboard::Key::Slash)                               \
    X(SDLK_0, ::za::Keyboard::Key::Num0)                                    \
    X(SDLK_1, ::za::Keyboard::Key::Num1)                                    \
    X(SDLK_2, ::za::Keyboard::Key::Num2)                                    \
    X(SDLK_3, ::za::Keyboard::Key::Num3)                                    \
    X(SDLK_4, ::za::Keyboard::Key::Num4)                                    \
    X(SDLK_5, ::za::Keyboard::Key::Num5)                                    \
    X(SDLK_6, ::za::Keyboard::Key::Num6)                                    \
    X(SDLK_7, ::za::Keyboard::Key::Num7)                                    \
    X(SDLK_8, ::za::Keyboard::Key::Num8)                                    \
    X(SDLK_9, ::za::Keyboard::Key::Num9)                                    \
    X(SDLK_COLON, ::za::Keyboard::Key::Colon)                               \
    X(SDLK_SEMICOLON, ::za::Keyboard::Key::Semicolon)                       \
    X(SDLK_LESS, ::za::Keyboard::Key::Less)                                 \
    X(SDLK_EQUALS, ::za::Keyboard::Key::Equal)                              \
    X(SDLK_GREATER, ::za::Keyboard::Key::Greater)                           \
    X(SDLK_QUESTION, ::za::Keyboard::Key::Question)                         \
    X(SDLK_AT, ::za::Keyboard::Key::At)                                     \
    X(SDLK_LEFTBRACKET, ::za::Keyboard::Key::LBracket)                      \
    X(SDLK_BACKSLASH, ::za::Keyboard::Key::Backslash)                       \
    X(SDLK_RIGHTBRACKET, ::za::Keyboard::Key::RBracket)                     \
    X(SDLK_CARET, ::za::Keyboard::Key::Caret)                               \
    X(SDLK_UNDERSCORE, ::za::Keyboard::Key::Underscore)                     \
    X(SDLK_GRAVE, ::za::Keyboard::Key::Grave)                               \
    X(SDLK_A, ::za::Keyboard::Key::A)                                       \
    X(SDLK_B, ::za::Keyboard::Key::B)                                       \
    X(SDLK_C, ::za::Keyboard::Key::C)                                       \
    X(SDLK_D, ::za::Keyboard::Key::D)                                       \
    X(SDLK_E, ::za::Keyboard::Key::E)                                       \
    X(SDLK_F, ::za::Keyboard::Key::F)                                       \
    X(SDLK_G, ::za::Keyboard::Key::G)                                       \
    X(SDLK_H, ::za::Keyboard::Key::H)                                       \
    X(SDLK_I, ::za::Keyboard::Key::I)                                       \
    X(SDLK_J, ::za::Keyboard::Key::J)                                       \
    X(SDLK_K, ::za::Keyboard::Key::K)                                       \
    X(SDLK_L, ::za::Keyboard::Key::L)                                       \
    X(SDLK_M, ::za::Keyboard::Key::M)                                       \
    X(SDLK_N, ::za::Keyboard::Key::N)                                       \
    X(SDLK_O, ::za::Keyboard::Key::O)                                       \
    X(SDLK_P, ::za::Keyboard::Key::P)                                       \
    X(SDLK_Q, ::za::Keyboard::Key::Q)                                       \
    X(SDLK_R, ::za::Keyboard::Key::R)                                       \
    X(SDLK_S, ::za::Keyboard::Key::S)                                       \
    X(SDLK_T, ::za::Keyboard::Key::T)                                       \
    X(SDLK_U, ::za::Keyboard::Key::U)                                       \
    X(SDLK_V, ::za::Keyboard::Key::V)                                       \
    X(SDLK_W, ::za::Keyboard::Key::W)                                       \
    X(SDLK_X, ::za::Keyboard::Key::X)                                       \
    X(SDLK_Y, ::za::Keyboard::Key::Y)                                       \
    X(SDLK_Z, ::za::Keyboard::Key::Z)                                       \
    X(SDLK_LEFTBRACE, ::za::Keyboard::Key::LBrace)                          \
    X(SDLK_PIPE, ::za::Keyboard::Key::Pipe)                                 \
    X(SDLK_RIGHTBRACE, ::za::Keyboard::Key::RBrace)                         \
    X(SDLK_TILDE, ::za::Keyboard::Key::Tilde)                               \
    X(SDLK_DELETE, ::za::Keyboard::Key::Delete)                             \
    X(SDLK_PLUSMINUS, ::za::Keyboard::Key::PlusMinus)                       \
    X(SDLK_CAPSLOCK, ::za::Keyboard::Key::CapsLock)                         \
    X(SDLK_F1, ::za::Keyboard::Key::F1)                                     \
    X(SDLK_F2, ::za::Keyboard::Key::F2)                                     \
    X(SDLK_F3, ::za::Keyboard::Key::F3)                                     \
    X(SDLK_F4, ::za::Keyboard::Key::F4)                                     \
    X(SDLK_F5, ::za::Keyboard::Key::F5)                                     \
    X(SDLK_F6, ::za::Keyboard::Key::F6)                                     \
    X(SDLK_F7, ::za::Keyboard::Key::F7)                                     \
    X(SDLK_F8, ::za::Keyboard::Key::F8)                                     \
    X(SDLK_F9, ::za::Keyboard::Key::F9)                                     \
    X(SDLK_F10, ::za::Keyboard::Key::F10)                                   \
    X(SDLK_F11, ::za::Keyboard::Key::F11)                                   \
    X(SDLK_F12, ::za::Keyboard::Key::F12)                                   \
    X(SDLK_PRINTSCREEN, ::za::Keyboard::Key::PrintScreen)                   \
    X(SDLK_SCROLLLOCK, ::za::Keyboard::Key::ScrollLock)                     \
    X(SDLK_PAUSE, ::za::Keyboard::Key::Pause)                               \
    X(SDLK_INSERT, ::za::Keyboard::Key::Insert)                             \
    X(SDLK_HOME, ::za::Keyboard::Key::Home)                                 \
    X(SDLK_PAGEUP, ::za::Keyboard::Key::PageUp)                             \
    X(SDLK_END, ::za::Keyboard::Key::End)                                   \
    X(SDLK_PAGEDOWN, ::za::Keyboard::Key::PageDown)                         \
    X(SDLK_RIGHT, ::za::Keyboard::Key::Right)                               \
    X(SDLK_LEFT, ::za::Keyboard::Key::Left)                                 \
    X(SDLK_DOWN, ::za::Keyboard::Key::Down)                                 \
    X(SDLK_UP, ::za::Keyboard::Key::Up)                                     \
    X(SDLK_NUMLOCKCLEAR, ::za::Keyboard::Key::NumLock)                      \
    X(SDLK_KP_DIVIDE, ::za::Keyboard::Key::Divide)                          \
    X(SDLK_KP_MULTIPLY, ::za::Keyboard::Key::Multiply)                      \
    X(SDLK_KP_MINUS, ::za::Keyboard::Key::Subtract)                         \
    X(SDLK_KP_PLUS, ::za::Keyboard::Key::Add)                               \
    X(SDLK_KP_ENTER, ::za::Keyboard::Key::NumpadEnter)                      \
    X(SDLK_KP_1, ::za::Keyboard::Key::Numpad1)                              \
    X(SDLK_KP_2, ::za::Keyboard::Key::Numpad2)                              \
    X(SDLK_KP_3, ::za::Keyboard::Key::Numpad3)                              \
    X(SDLK_KP_4, ::za::Keyboard::Key::Numpad4)                              \
    X(SDLK_KP_5, ::za::Keyboard::Key::Numpad5)                              \
    X(SDLK_KP_6, ::za::Keyboard::Key::Numpad6)                              \
    X(SDLK_KP_7, ::za::Keyboard::Key::Numpad7)                              \
    X(SDLK_KP_8, ::za::Keyboard::Key::Numpad8)                              \
    X(SDLK_KP_9, ::za::Keyboard::Key::Numpad9)                              \
    X(SDLK_KP_0, ::za::Keyboard::Key::Numpad0)                              \
    X(SDLK_KP_PERIOD, ::za::Keyboard::Key::NumpadPeriod)                    \
    X(SDLK_APPLICATION, ::za::Keyboard::Key::Application)                   \
    X(SDLK_POWER, ::za::Keyboard::Key::Power)                               \
    X(SDLK_KP_EQUALS, ::za::Keyboard::Key::NumpadEqual)                     \
    X(SDLK_F13, ::za::Keyboard::Key::F13)                                   \
    X(SDLK_F14, ::za::Keyboard::Key::F14)                                   \
    X(SDLK_F15, ::za::Keyboard::Key::F15)                                   \
    X(SDLK_F16, ::za::Keyboard::Key::F16)                                   \
    X(SDLK_F17, ::za::Keyboard::Key::F17)                                   \
    X(SDLK_F18, ::za::Keyboard::Key::F18)                                   \
    X(SDLK_F19, ::za::Keyboard::Key::F19)                                   \
    X(SDLK_F20, ::za::Keyboard::Key::F20)                                   \
    X(SDLK_F21, ::za::Keyboard::Key::F21)                                   \
    X(SDLK_F22, ::za::Keyboard::Key::F22)                                   \
    X(SDLK_F23, ::za::Keyboard::Key::F23)                                   \
    X(SDLK_F24, ::za::Keyboard::Key::F24)                                   \
    X(SDLK_EXECUTE, ::za::Keyboard::Key::Execute)                           \
    X(SDLK_HELP, ::za::Keyboard::Key::Help)                                 \
    X(SDLK_MENU, ::za::Keyboard::Key::Menu)                                 \
    X(SDLK_SELECT, ::za::Keyboard::Key::Select)                             \
    X(SDLK_STOP, ::za::Keyboard::Key::Stop)                                 \
    X(SDLK_AGAIN, ::za::Keyboard::Key::Redo)                                \
    X(SDLK_UNDO, ::za::Keyboard::Key::Undo)                                 \
    X(SDLK_CUT, ::za::Keyboard::Key::Cut)                                   \
    X(SDLK_COPY, ::za::Keyboard::Key::Copy)                                 \
    X(SDLK_PASTE, ::za::Keyboard::Key::Paste)                               \
    X(SDLK_FIND, ::za::Keyboard::Key::Find)                                 \
    X(SDLK_MUTE, ::za::Keyboard::Key::VolumeMute)                           \
    X(SDLK_VOLUMEUP, ::za::Keyboard::Key::VolumeUp)                         \
    X(SDLK_VOLUMEDOWN, ::za::Keyboard::Key::VolumeDown)                     \
    X(SDLK_KP_COMMA, ::za::Keyboard::Key::NumpadComma)                      \
    X(SDLK_KP_EQUALSAS400, ::za::Keyboard::Key::NumpadEqualAs400)           \
    X(SDLK_ALTERASE, ::za::Keyboard::Key::AltErase)                         \
    X(SDLK_SYSREQ, ::za::Keyboard::Key::SysReq)                             \
    X(SDLK_CANCEL, ::za::Keyboard::Key::Cancel)                             \
    X(SDLK_CLEAR, ::za::Keyboard::Key::Clear)                               \
    X(SDLK_PRIOR, ::za::Keyboard::Key::Prior)                               \
    X(SDLK_RETURN2, ::za::Keyboard::Key::Enter2)                            \
    X(SDLK_SEPARATOR, ::za::Keyboard::Key::Separator)                       \
    X(SDLK_OUT, ::za::Keyboard::Key::Out)                                   \
    X(SDLK_OPER, ::za::Keyboard::Key::Oper)                                 \
    X(SDLK_CLEARAGAIN, ::za::Keyboard::Key::ClearAgain)                     \
    X(SDLK_CRSEL, ::za::Keyboard::Key::CrSel)                               \
    X(SDLK_EXSEL, ::za::Keyboard::Key::ExSel)                               \
    X(SDLK_KP_00, ::za::Keyboard::Key::Numpad00)                            \
    X(SDLK_KP_000, ::za::Keyboard::Key::Numpad000)                          \
    X(SDLK_THOUSANDSSEPARATOR, ::za::Keyboard::Key::ThousandsSeparator)     \
    X(SDLK_DECIMALSEPARATOR, ::za::Keyboard::Key::DecimalSeparator)         \
    X(SDLK_CURRENCYUNIT, ::za::Keyboard::Key::CurrencyUnit)                 \
    X(SDLK_CURRENCYSUBUNIT, ::za::Keyboard::Key::CurrencySubUnit)           \
    X(SDLK_KP_LEFTPAREN, ::za::Keyboard::Key::NumpadLParenthesis)           \
    X(SDLK_KP_RIGHTPAREN, ::za::Keyboard::Key::NumpadRParenthesis)          \
    X(SDLK_KP_LEFTBRACE, ::za::Keyboard::Key::NumpadLBrace)                 \
    X(SDLK_KP_RIGHTBRACE, ::za::Keyboard::Key::NumpadRBrace)                \
    X(SDLK_KP_TAB, ::za::Keyboard::Key::NumpadTab)                          \
    X(SDLK_KP_BACKSPACE, ::za::Keyboard::Key::NumpadBackspace)              \
    X(SDLK_KP_A, ::za::Keyboard::Key::NumpadA)                              \
    X(SDLK_KP_B, ::za::Keyboard::Key::NumpadB)                              \
    X(SDLK_KP_C, ::za::Keyboard::Key::NumpadC)                              \
    X(SDLK_KP_D, ::za::Keyboard::Key::NumpadD)                              \
    X(SDLK_KP_E, ::za::Keyboard::Key::NumpadE)                              \
    X(SDLK_KP_F, ::za::Keyboard::Key::NumpadF)                              \
    X(SDLK_KP_XOR, ::za::Keyboard::Key::NumpadXor)                          \
    X(SDLK_KP_POWER, ::za::Keyboard::Key::NumpadPower)                      \
    X(SDLK_KP_PERCENT, ::za::Keyboard::Key::NumpadPercent)                  \
    X(SDLK_KP_LESS, ::za::Keyboard::Key::NumpadLess)                        \
    X(SDLK_KP_GREATER, ::za::Keyboard::Key::NumpadGreater)                  \
    X(SDLK_KP_AMPERSAND, ::za::Keyboard::Key::NumpadAmpersand)              \
    X(SDLK_KP_DBLAMPERSAND, ::za::Keyboard::Key::NumpadDoubleAmpersand)     \
    X(SDLK_KP_VERTICALBAR, ::za::Keyboard::Key::NumpadVerticalBar)          \
    X(SDLK_KP_DBLVERTICALBAR, ::za::Keyboard::Key::NumpadDoubleVerticalBar) \
    X(SDLK_KP_COLON, ::za::Keyboard::Key::NumpadColon)                      \
    X(SDLK_KP_HASH, ::za::Keyboard::Key::NumpadHash)                        \
    X(SDLK_KP_SPACE, ::za::Keyboard::Key::NumpadSpace)                      \
    X(SDLK_KP_AT, ::za::Keyboard::Key::NumpadAt)                            \
    X(SDLK_KP_EXCLAM, ::za::Keyboard::Key::NumpadExclamation)               \
    X(SDLK_KP_MEMSTORE, ::za::Keyboard::Key::NumpadMemStore)                \
    X(SDLK_KP_MEMRECALL, ::za::Keyboard::Key::NumpadMemRecall)              \
    X(SDLK_KP_MEMCLEAR, ::za::Keyboard::Key::NumpadMemClear)                \
    X(SDLK_KP_MEMADD, ::za::Keyboard::Key::NumpadMemAdd)                    \
    X(SDLK_KP_MEMSUBTRACT, ::za::Keyboard::Key::NumpadMemSubtract)          \
    X(SDLK_KP_MEMMULTIPLY, ::za::Keyboard::Key::NumpadMemMultiply)          \
    X(SDLK_KP_MEMDIVIDE, ::za::Keyboard::Key::NumpadMemDivide)              \
    X(SDLK_KP_PLUSMINUS, ::za::Keyboard::Key::NumpadPlusMinus)              \
    X(SDLK_KP_CLEAR, ::za::Keyboard::Key::NumpadClear)                      \
    X(SDLK_KP_CLEARENTRY, ::za::Keyboard::Key::NumpadClearEntry)            \
    X(SDLK_KP_BINARY, ::za::Keyboard::Key::NumpadBinary)                    \
    X(SDLK_KP_OCTAL, ::za::Keyboard::Key::NumpadOctal)                      \
    X(SDLK_KP_DECIMAL, ::za::Keyboard::Key::NumpadDecimal)                  \
    X(SDLK_KP_HEXADECIMAL, ::za::Keyboard::Key::NumpadHexadecimal)          \
    X(SDLK_LCTRL, ::za::Keyboard::Key::LControl)                            \
    X(SDLK_LSHIFT, ::za::Keyboard::Key::LShift)                             \
    X(SDLK_LALT, ::za::Keyboard::Key::LAlt)                                 \
    X(SDLK_LGUI, ::za::Keyboard::Key::LSystem)                              \
    X(SDLK_RCTRL, ::za::Keyboard::Key::RControl)                            \
    X(SDLK_RSHIFT, ::za::Keyboard::Key::RShift)                             \
    X(SDLK_RALT, ::za::Keyboard::Key::RAlt)                                 \
    X(SDLK_RGUI, ::za::Keyboard::Key::RSystem)                              \
    X(SDLK_MODE, ::za::Keyboard::Key::ModeChange)                           \
    X(SDLK_SLEEP, ::za::Keyboard::Key::Sleep)                               \
    X(SDLK_WAKE, ::za::Keyboard::Key::Wake)                                 \
    X(SDLK_CHANNEL_INCREMENT, ::za::Keyboard::Key::ChannelIncrement)        \
    X(SDLK_CHANNEL_DECREMENT, ::za::Keyboard::Key::ChannelDecrement)        \
    X(SDLK_MEDIA_PLAY, ::za::Keyboard::Key::MediaPlay)                      \
    X(SDLK_MEDIA_PAUSE, ::za::Keyboard::Key::MediaPause)                    \
    X(SDLK_MEDIA_RECORD, ::za::Keyboard::Key::MediaRecord)                  \
    X(SDLK_MEDIA_FAST_FORWARD, ::za::Keyboard::Key::MediaFastForward)       \
    X(SDLK_MEDIA_REWIND, ::za::Keyboard::Key::MediaRewind)                  \
    X(SDLK_MEDIA_NEXT_TRACK, ::za::Keyboard::Key::MediaNextTrack)           \
    X(SDLK_MEDIA_PREVIOUS_TRACK, ::za::Keyboard::Key::MediaPreviousTrack)   \
    X(SDLK_MEDIA_STOP, ::za::Keyboard::Key::MediaStop)                      \
    X(SDLK_MEDIA_EJECT, ::za::Keyboard::Key::MediaEject)                    \
    X(SDLK_MEDIA_PLAY_PAUSE, ::za::Keyboard::Key::MediaPlayPause)           \
    X(SDLK_MEDIA_SELECT, ::za::Keyboard::Key::LaunchMediaSelect)            \
    X(SDLK_AC_NEW, ::za::Keyboard::Key::AcNew)                              \
    X(SDLK_AC_OPEN, ::za::Keyboard::Key::AcOpen)                            \
    X(SDLK_AC_CLOSE, ::za::Keyboard::Key::AcClose)                          \
    X(SDLK_AC_EXIT, ::za::Keyboard::Key::AcExit)                            \
    X(SDLK_AC_SAVE, ::za::Keyboard::Key::AcSave)                            \
    X(SDLK_AC_PRINT, ::za::Keyboard::Key::AcPrint)                          \
    X(SDLK_AC_PROPERTIES, ::za::Keyboard::Key::AcProperties)                \
    X(SDLK_AC_SEARCH, ::za::Keyboard::Key::Search)                          \
    X(SDLK_AC_HOME, ::za::Keyboard::Key::HomePage)                          \
    X(SDLK_AC_BACK, ::za::Keyboard::Key::Back)                              \
    X(SDLK_AC_FORWARD, ::za::Keyboard::Key::Forward)                        \
    X(SDLK_AC_STOP, ::za::Keyboard::Key::AcStop)                            \
    X(SDLK_AC_REFRESH, ::za::Keyboard::Key::Refresh)                        \
    X(SDLK_AC_BOOKMARKS, ::za::Keyboard::Key::Favorites)                    \
    X(SDLK_SOFTLEFT, ::za::Keyboard::Key::SoftLeft)                         \
    X(SDLK_SOFTRIGHT, ::za::Keyboard::Key::SoftRight)                       \
    X(SDLK_CALL, ::za::Keyboard::Key::Call)                                 \
    X(SDLK_ENDCALL, ::za::Keyboard::Key::EndCall)                           \
    X(SDLK_LEFT_TAB, ::za::Keyboard::Key::LTab)                             \
    X(SDLK_LEVEL5_SHIFT, ::za::Keyboard::Key::Level5Shift)                  \
    X(SDLK_MULTI_KEY_COMPOSE, ::za::Keyboard::Key::MultiKeyCompose)         \
    X(SDLK_LMETA, ::za::Keyboard::Key::LMeta)                               \
    X(SDLK_RMETA, ::za::Keyboard::Key::RMeta)                               \
    X(SDLK_LHYPER, ::za::Keyboard::Key::LHyper)                             \
    X(SDLK_RHYPER, ::za::Keyboard::Key::RHyper)


namespace za::priv
{
////////////////////////////////////////////////////////////
[[nodiscard, gnu::const]] Keyboard::Scan mapSDLScancodeToSFML(const SDL_Scancode sdlCode) noexcept
{
    return static_cast<Keyboard::Scan>(sdlCode);
}


////////////////////////////////////////////////////////////
[[nodiscard, gnu::const]] SDL_Scancode mapSFMLScancodeToSDL(const Keyboard::Scan scanCode) noexcept
{
    return static_cast<SDL_Scancode>(scanCode);
}


////////////////////////////////////////////////////////////
[[nodiscard, gnu::const]] Keyboard::Key mapSDLKeycodeToSFML(const SDL_Keycode sdlKey) noexcept
{
    switch (sdlKey)
    {
#define X(sdlKey, zancleKey) \
    case sdlKey:           \
        return zancleKey;
        ZA_PRIV_SFML_SDL_KEYCODE_MAPPING
#undef X
    }

    return Keyboard::Key::Unknown;
}


////////////////////////////////////////////////////////////
[[nodiscard, gnu::const]] SDL_Keycode mapSFMLKeycodeToSDL(const Keyboard::Key key) noexcept
{
    switch (key)
    {
#define X(sdlKey, zancleKey) \
    case zancleKey:          \
        return sdlKey;
        ZA_PRIV_SFML_SDL_KEYCODE_MAPPING
#undef X
    }

    return SDLK_UNKNOWN;
}


////////////////////////////////////////////////////////////
#undef ZA_PRIV_SFML_SDL_KEYCODE_MAPPING


////////////////////////////////////////////////////////////
[[nodiscard, gnu::const]] SDL_SystemCursor cursorTypeToSDLCursor(const za::Cursor::Type type) noexcept
{
    // clang-format off
    switch (type)
    {
        case za::Cursor::Type::Arrow:                  return SDL_SYSTEM_CURSOR_DEFAULT;
        case za::Cursor::Type::ArrowWait:              return SDL_SYSTEM_CURSOR_PROGRESS;
        case za::Cursor::Type::Wait:                   return SDL_SYSTEM_CURSOR_WAIT;
        case za::Cursor::Type::Text:                   return SDL_SYSTEM_CURSOR_TEXT;
        case za::Cursor::Type::Hand:                   return SDL_SYSTEM_CURSOR_POINTER;
        case za::Cursor::Type::SizeHorizontal:         return SDL_SYSTEM_CURSOR_EW_RESIZE;
        case za::Cursor::Type::SizeVertical:           return SDL_SYSTEM_CURSOR_NS_RESIZE;
        case za::Cursor::Type::SizeTopLeftBottomRight: return SDL_SYSTEM_CURSOR_NWSE_RESIZE;
        case za::Cursor::Type::SizeBottomLeftTopRight: return SDL_SYSTEM_CURSOR_NESW_RESIZE;
        case za::Cursor::Type::SizeLeft:               return SDL_SYSTEM_CURSOR_W_RESIZE;
        case za::Cursor::Type::SizeRight:              return SDL_SYSTEM_CURSOR_E_RESIZE;
        case za::Cursor::Type::SizeTop:                return SDL_SYSTEM_CURSOR_N_RESIZE;
        case za::Cursor::Type::SizeBottom:             return SDL_SYSTEM_CURSOR_S_RESIZE;
        case za::Cursor::Type::SizeTopLeft:            return SDL_SYSTEM_CURSOR_NW_RESIZE;
        case za::Cursor::Type::SizeBottomRight:        return SDL_SYSTEM_CURSOR_SE_RESIZE;
        case za::Cursor::Type::SizeBottomLeft:         return SDL_SYSTEM_CURSOR_SW_RESIZE;
        case za::Cursor::Type::SizeTopRight:           return SDL_SYSTEM_CURSOR_NE_RESIZE;
        case za::Cursor::Type::SizeAll:                return SDL_SYSTEM_CURSOR_MOVE;
        case za::Cursor::Type::Cross:                  return SDL_SYSTEM_CURSOR_CROSSHAIR;
        case za::Cursor::Type::NotAllowed:             return SDL_SYSTEM_CURSOR_NOT_ALLOWED;
        case za::Cursor::Type::Help:                   break; // not supported
    }
    // clang-format on

    return SDL_SYSTEM_CURSOR_DEFAULT;
}


////////////////////////////////////////////////////////////
[[nodiscard]] SDL_PropertiesID makeSDLWindowPropertiesFromHandle([[maybe_unused]] const char* const currentVideoDriver,
                                                                 const WindowHandle                 handle)
{
    const SDL_PropertiesID props = SDL_CreateProperties();

#if defined(ZA_SYSTEM_WINDOWS)
    SDL_SetPointerProperty(props, SDL_PROP_WINDOW_CREATE_WIN32_HWND_POINTER, handle);
    SDL_SetNumberProperty(props, SDL_PROP_WINDOW_CREATE_FLAGS_NUMBER, static_cast<Sint64>(SDL_WINDOW_OPENGL));
#elif defined(ZA_SYSTEM_LINUX_OR_BSD)
    if (currentVideoDriver != nullptr && ZB_STRCMP(currentVideoDriver, "wayland") == 0)
        SDL_SetPointerProperty(props, SDL_PROP_WINDOW_CREATE_WAYLAND_WL_SURFACE_POINTER, reinterpret_cast<void*>(handle));
    else
        SDL_SetNumberProperty(props, SDL_PROP_WINDOW_CREATE_X11_WINDOW_NUMBER, static_cast<Sint64>(handle));
#elif defined(ZA_SYSTEM_MACOS)
    SDL_SetPointerProperty(props, SDL_PROP_WINDOW_CREATE_COCOA_WINDOW_POINTER, handle);
#elif defined(ZA_SYSTEM_IOS)
    // TODO P0: doesn't seem to be implemented in SDL
#elif defined(ZA_SYSTEM_ANDROID)
    // TODO P0: doesn't seem to be implemented in SDL
#elif defined(ZA_SYSTEM_EMSCRIPTEN)
    SDL_SetStringProperty(props, SDL_PROP_WINDOW_CREATE_EMSCRIPTEN_CANVAS_ID_STRING, handle);
#endif

    return props;
}


////////////////////////////////////////////////////////////
[[nodiscard]] SDL_PropertiesID makeSDLWindowPropertiesFromWindowSettings(const WindowSettings& windowSettings)
{
    const auto flags = makeSDLWindowFlagsFromWindowSettings(windowSettings);

    const SDL_PropertiesID props = SDL_CreateProperties();

    SDL_SetNumberProperty(props, SDL_PROP_WINDOW_CREATE_FLAGS_NUMBER, static_cast<Sint64>(flags));
    SDL_SetNumberProperty(props, SDL_PROP_WINDOW_CREATE_WIDTH_NUMBER, windowSettings.size.x);
    SDL_SetNumberProperty(props, SDL_PROP_WINDOW_CREATE_HEIGHT_NUMBER, windowSettings.size.y);
    SDL_SetStringProperty(props, SDL_PROP_WINDOW_CREATE_TITLE_STRING, windowSettings.title.cStr());

    if (windowSettings.position.hasValue())
    {
        SDL_SetNumberProperty(props, SDL_PROP_WINDOW_CREATE_X_NUMBER, windowSettings.position->x);
        SDL_SetNumberProperty(props, SDL_PROP_WINDOW_CREATE_Y_NUMBER, windowSettings.position->y);
    }

    if (!windowSettings.visible)
        SDL_SetBooleanProperty(props, SDL_PROP_WINDOW_CREATE_HIDDEN_BOOLEAN, true);

    // TODO P1: review possible multi-canvas support
    /*
        static int i = 0;

        if (i++ != 0)
        {
            char id[64];
            SDL_snprintf(id, sizeof(id), "#canvas%d", i + 1);
            SDL_SetStringProperty(props, SDL_PROP_WINDOW_CREATE_EMSCRIPTEN_CANVAS_ID_STRING, id);
            SDL_SetStringProperty(props, SDL_PROP_WINDOW_CREATE_EMSCRIPTEN_KEYBOARD_ELEMENT_STRING, id);
        }
    */

    return props;
}


////////////////////////////////////////////////////////////
[[nodiscard, gnu::pure]] SDL_WindowFlags makeSDLWindowFlagsFromWindowSettings(const WindowSettings& windowSettings) noexcept
{
    SDL_WindowFlags flags{};

    flags |= SDL_WINDOW_OPENGL;

    if (windowSettings.fullscreen)
        flags |= SDL_WINDOW_FULLSCREEN;

    if (windowSettings.resizable)
        flags |= SDL_WINDOW_RESIZABLE;

    if (!windowSettings.hasTitlebar)
        flags |= SDL_WINDOW_BORDERLESS;

    return flags;
}


////////////////////////////////////////////////////////////
[[nodiscard, gnu::const]] Mouse::Button getButtonFromSDLButton(const zb::U8 sdlButton) noexcept
{
    // clang-format off
    switch (sdlButton)
    {
        case SDL_BUTTON_LEFT:   return Mouse::Button::Left;
        case SDL_BUTTON_MIDDLE: return Mouse::Button::Middle;
        case SDL_BUTTON_RIGHT:  return Mouse::Button::Right;
        case SDL_BUTTON_X1:     return Mouse::Button::Extra1;
        case SDL_BUTTON_X2:     return Mouse::Button::Extra2;
    }
    // clang-format on

    ZB_ASSERT(false);
    return Mouse::Button::Left;
}


////////////////////////////////////////////////////////////
[[nodiscard, gnu::const]] zb::U8 getSDLButtonFromSFMLButton(const Mouse::Button button) noexcept
{
    // clang-format off
    switch (button)
    {
        case Mouse::Button::Left:   return SDL_BUTTON_LEFT;
        case Mouse::Button::Middle: return SDL_BUTTON_MIDDLE;
        case Mouse::Button::Right:  return SDL_BUTTON_RIGHT;
        case Mouse::Button::Extra1: return SDL_BUTTON_X1;
        case Mouse::Button::Extra2: return SDL_BUTTON_X2;
    }
    // clang-format on

    ZB_ASSERT(false);
    return SDL_BUTTON_LEFT;
}


////////////////////////////////////////////////////////////
DisplayOrientation mapSDLDisplayOrientationToSFML(const SDL_DisplayOrientation displayOrientation)
{
    return static_cast<DisplayOrientation>(displayOrientation);
}


////////////////////////////////////////////////////////////
SDLLayer::SDLLayer()
{
#ifdef ZA_SYSTEM_EMSCRIPTEN
    // Ensures window position is synced with DOM position on Emscripten.
    if (!SDL_SetHint(SDL_HINT_VIDEO_SYNC_WINDOW_OPERATIONS, "1"))
        errMsg("`SDL_SetHint(SDL_HINT_VIDEO_SYNC_WINDOW_OPERATIONS)` failed: {}", SDL_GetError());

    // Tell SDL3 not to call `emscripten_sleep(0)` inside `SDL_GL_SwapWindow`.
    // Our build links with `-sASYNCIFY_IGNORE_INDIRECT=1`, which means
    // asyncify yields cannot occur inside indirect calls (and SDL invokes
    // its `GL_SwapWindow` through a function pointer). Without this hint,
    // each `RenderWindow::display()` traps with `unreachable`. Browsers
    // already drive frame timing through `requestAnimationFrame`, so the
    // implicit sleep was redundant for us anyway.
    if (!SDL_SetHint(SDL_HINT_EMSCRIPTEN_ASYNCIFY, "0"))
        errMsg("`SDL_SetHint(SDL_HINT_EMSCRIPTEN_ASYNCIFY)` failed: {}", SDL_GetError());
#endif

    if (!SDL_InitSubSystem(SDL_INIT_VIDEO))
    {
        errMsg("`SDL_Init` failed: {}", SDL_GetError());
        zb::abort();
    }
}


////////////////////////////////////////////////////////////
SDLLayer::~SDLLayer()
{
    SDL_QuitSubSystem(SDL_INIT_VIDEO);
}


////////////////////////////////////////////////////////////
SDLAllocatedArray<SDL_DisplayID> SDLLayer::getDisplays() const
{
    int            displayCount = 0;
    SDL_DisplayID* displays     = SDL_GetDisplays(&displayCount);

    if (displays == nullptr)
    {
        errMsg("`SDL_GetDisplays` failed: {}", SDL_GetError());
        return nullptr;
    }

    return SDLAllocatedArray<SDL_DisplayID>{SDLUPtr<SDL_DisplayID>{displays}, static_cast<zb::SizeT>(displayCount)};
}


////////////////////////////////////////////////////////////
SDLAllocatedArray<SDL_DisplayMode*> SDLLayer::getFullscreenDisplayModesForDisplay(const SDL_DisplayID displayId)
{
    ZB_ASSERT(displayId != 0u);

    int               modeCount = 0;
    SDL_DisplayMode** modes     = SDL_GetFullscreenDisplayModes(displayId, &modeCount);

    if (modes == nullptr)
    {
        errMsg("`SDL_GetFullscreenDisplayModes` failed for display {}: {}", displayId, SDL_GetError());
        return nullptr;
    }

    return SDLAllocatedArray<SDL_DisplayMode*>{SDLUPtr<SDL_DisplayMode*>(modes), static_cast<zb::SizeT>(modeCount)};
}


////////////////////////////////////////////////////////////
const SDL_PixelFormatDetails* SDLLayer::getPixelFormatDetails(const SDL_PixelFormat format) const
{
    ZB_ASSERT(format != 0);

    const auto* result = SDL_GetPixelFormatDetails(format);

    if (result == nullptr)
    {
        errMsg("`SDL_GetPixelFormatDetails` failed for format {}: {}", static_cast<int>(format), SDL_GetError());
        return nullptr;
    }

    return result;
}


////////////////////////////////////////////////////////////
const SDL_DisplayMode* SDLLayer::getDesktopDisplayMode(const SDL_DisplayID displayId) const
{
    ZB_ASSERT(displayId != 0u);

    const auto* result = SDL_GetDesktopDisplayMode(displayId);

    if (result == nullptr)
    {
        errMsg("`SDL_GetDesktopDisplayMode` failed for display {}: {}", displayId, SDL_GetError());
        return nullptr;
    }

    return result;
}


////////////////////////////////////////////////////////////
const SDL_DisplayMode* SDLLayer::getPrimaryDisplayDesktopDisplayMode() const
{
    const auto displays = getDisplays();

    if (displays.empty())
    {
        errMsg("`getPrimaryDisplayDesktopDisplayMode` failed: no displays");
        return nullptr;
    }

    if (!displays.valid())
    {
        errMsg("`getPrimaryDisplayDesktopDisplayMode` failed: invalid displays");
        return nullptr;
    }

    return getDesktopDisplayMode(displays[0]);
}


////////////////////////////////////////////////////////////
SDLAllocatedArray<SDL_TouchID> SDLLayer::getTouchDevices()
{
    int          idCount = 0;
    SDL_TouchID* ids     = SDL_GetTouchDevices(&idCount);

    if (ids == nullptr)
    {
        errMsg("`SDL_GetTouchDevices` failed: {}", SDL_GetError());
        return nullptr;
    }

    return SDLAllocatedArray<SDL_TouchID>{SDLUPtr<SDL_TouchID>(ids), static_cast<zb::SizeT>(idCount)};
}


////////////////////////////////////////////////////////////
SDLAllocatedArray<SDL_Finger*> SDLLayer::getTouchFingers(const SDL_TouchID touchDeviceId)
{
    ZB_ASSERT(touchDeviceId != 0u);

    int          fingerCount = 0;
    SDL_Finger** fingers     = SDL_GetTouchFingers(touchDeviceId, &fingerCount);

    if (fingers == nullptr)
    {
        errMsg("`SDL_GetTouchFingers` failed for touch device {}: {}", touchDeviceId, SDL_GetError());
        return nullptr;
    }

    return SDLAllocatedArray<SDL_Finger*>{SDLUPtr<SDL_Finger*>(fingers), static_cast<zb::SizeT>(fingerCount)};
}


////////////////////////////////////////////////////////////
SDL_TouchDeviceType SDLLayer::getTouchDeviceType(const SDL_TouchID touchDeviceId)
{
    ZB_ASSERT(touchDeviceId != 0u);

    return SDL_GetTouchDeviceType(touchDeviceId);
}


////////////////////////////////////////////////////////////
const char* SDLLayer::getTouchDeviceName(const SDL_TouchID touchDeviceId)
{
    ZB_ASSERT(touchDeviceId != 0u);

    return SDL_GetTouchDeviceName(touchDeviceId);
}


////////////////////////////////////////////////////////////
Keyboard::Key SDLLayer::localizeScancode(const Keyboard::Scancode code) const noexcept
{
    const SDL_Scancode sdlScancode = mapSFMLScancodeToSDL(code);
    const SDL_Keycode  sdlKey      = SDL_GetKeyFromScancode(sdlScancode, SDL_GetModState(), true);

    return mapSDLKeycodeToSFML(sdlKey);
}


////////////////////////////////////////////////////////////
Keyboard::Scancode SDLLayer::delocalizeScancode(const Keyboard::Key key) const noexcept
{
    const SDL_Keycode sdlKey = mapSFMLKeycodeToSDL(key);

    SDL_Keymod         mod{};
    const SDL_Scancode sdlScancode = SDL_GetScancodeFromKey(sdlKey, &mod);

    return mapSDLScancodeToSFML(sdlScancode);
}


////////////////////////////////////////////////////////////
bool SDLLayer::isKeyPressedByScancode(const Keyboard::Scancode code) const noexcept
{
    const bool* keyboardState = SDL_GetKeyboardState(nullptr);
    ZB_ASSERT(keyboardState != nullptr);

    return keyboardState[mapSFMLScancodeToSDL(code)];
}


////////////////////////////////////////////////////////////
const char* SDLLayer::getScancodeDescription(const Keyboard::Scancode code) const noexcept
{
    return SDL_GetKeyName(mapSFMLKeycodeToSDL(Keyboard::localize(code)));
}


////////////////////////////////////////////////////////////
void SDLLayer::setVirtualKeyboardVisible(const bool visible) const noexcept
{
    // No-op on platforms without a screen keyboard; on desktop, `SDL_StartTextInput`
    // is already enabled at window creation and we don't want to toggle text input
    // events as a side-effect of this call.
    if (!SDL_HasScreenKeyboardSupport())
        return;

    SDL_Window* const focused = SDL_GetKeyboardFocus();
    if (focused == nullptr)
        return;

    const bool ok = visible ? SDL_StartTextInput(focused) : SDL_StopTextInput(focused);
    if (!ok)
        errMsg("Failed to {} virtual keyboard: {}", (visible ? "show" : "hide"), SDL_GetError());
}


////////////////////////////////////////////////////////////
Vec2i SDLLayer::getGlobalMousePosition() const noexcept
{
    Vec2f result;
    SDL_GetGlobalMouseState(&result.x, &result.y);
    return result.toVec2i();
}


////////////////////////////////////////////////////////////
bool SDLLayer::setGlobalMousePosition(const Vec2i position) const noexcept
{
    if (!SDL_WarpMouseGlobal(static_cast<float>(position.x), static_cast<float>(position.y)))
    {
        errMsg("`SDL_WarpMouseGlobal` failed: {}", SDL_GetError());
        return false;
    }

    return true;
}


////////////////////////////////////////////////////////////
Utf8String SDLLayer::getClipboardString() const noexcept
{
    if (!SDL_HasClipboardText())
        return Utf8String{};

    char* clipboardText = SDL_GetClipboardText();
    ZB_SCOPE_GUARD({ SDL_free(static_cast<void*>(clipboardText)); });

    if (ZB_STRCMP(clipboardText, "") == 0)
    {
        errMsg("`SDL_GetClipboardText` failed: {}", SDL_GetError());
        return Utf8String{};
    }

    return Utf8String{clipboardText, ZB_STRLEN(clipboardText)};
}


////////////////////////////////////////////////////////////
bool SDLLayer::setClipboardString(const Utf8String& string) const noexcept
{
    if (!SDL_SetClipboardText(string.cStr()))
    {
        errMsg("`SDL_SetClipboardText` failed: {}", SDL_GetError());
        return false;
    }

    return true;
}


////////////////////////////////////////////////////////////
float SDLLayer::getDisplayContentScale(const SDL_DisplayID displayID) const
{
    ZB_ASSERT(displayID != 0u);

    const float result = SDL_GetDisplayContentScale(displayID);
    if (result == 0.f)
    {
        errMsg("`SDL_GetDisplayContentScale` failed:{}", SDL_GetError());
        return 1.f;
    }

    return result;
}


////////////////////////////////////////////////////////////
Utf8String SDLLayer::getDisplayName(const SDL_DisplayID displayID) const
{
    ZB_ASSERT(displayID != 0u);

    const char* const name = SDL_GetDisplayName(displayID);
    if (name == nullptr)
    {
        errMsg("`SDL_GetDisplayName` failed: {}", SDL_GetError());
        return Utf8String{};
    }

    return Utf8String{name, ZB_STRLEN(name)};
}


////////////////////////////////////////////////////////////
Rect2i SDLLayer::getDisplayBounds(const SDL_DisplayID displayID) const
{
    ZB_ASSERT(displayID != 0u);

    SDL_Rect out{};
    if (!SDL_GetDisplayBounds(displayID, &out))
    {
        errMsg("`SDL_GetDisplayBounds` failed: {}", SDL_GetError());
        return Rect2i{};
    }

    return Rect2i{{out.x, out.y}, {out.w, out.h}};
}


////////////////////////////////////////////////////////////
Rect2i SDLLayer::getDisplayUsableBounds(const SDL_DisplayID displayID) const
{
    ZB_ASSERT(displayID != 0u);

    SDL_Rect out{};
    if (!SDL_GetDisplayUsableBounds(displayID, &out))
    {
        errMsg("`SDL_GetDisplayUsableBounds` failed: {}", SDL_GetError());
        return Rect2i{};
    }

    return Rect2i{{out.x, out.y}, {out.w, out.h}};
}


////////////////////////////////////////////////////////////
DisplayOrientation SDLLayer::getNaturalDisplayOrientation(const SDL_DisplayID displayID) const
{
    ZB_ASSERT(displayID != 0u);
    return mapSDLDisplayOrientationToSFML(SDL_GetNaturalDisplayOrientation(displayID));
}


////////////////////////////////////////////////////////////
DisplayOrientation SDLLayer::getCurrentDisplayOrientation(const SDL_DisplayID displayID) const
{
    ZB_ASSERT(displayID != 0u);
    return mapSDLDisplayOrientationToSFML(SDL_GetCurrentDisplayOrientation(displayID));
}


////////////////////////////////////////////////////////////
float SDLLayer::getPrimaryDisplayContentScale() const
{
    const auto displays = getDisplays();

    if (!displays.valid())
    {
        errMsg("`getDisplayContentScale` failed: could not get displays");
        return 1.f;
    }

    if (displays.size() == 0)
    {
        errMsg("`getDisplayContentScale` failed: no displays found");
        return 1.f;
    }

    return getDisplayContentScale(displays[0]);
}


////////////////////////////////////////////////////////////
float SDLLayer::getDisplayScale(SDL_Window& window) const
{
    const float result = SDL_GetWindowDisplayScale(&window);

    if (result == 0.f)
    {
        errMsg("`SDL_GetWindowDisplayScale` failed: {}", SDL_GetError());
        return 1.f;
    }

    return result;
}


////////////////////////////////////////////////////////////
SDLSurfaceUPtr SDLLayer::createSurfaceFromPixels(const zb::U8* pixels, Vec2u size) const
{
    ZB_ASSERT(pixels != nullptr);

    SDL_Surface* surface = SDL_CreateSurfaceFrom(static_cast<int>(size.x),
                                                 static_cast<int>(size.y),
                                                 SDL_PIXELFORMAT_RGBA32,
                                                 const_cast<void*>(static_cast<const void*>(pixels)),
                                                 static_cast<int>(size.x * 4));

    if (surface == nullptr)
    {
        errMsg("`SDL_CreateSurfaceFrom` failed: {}", SDL_GetError());
        return nullptr;
    }

    return SDLSurfaceUPtr{surface};
}


////////////////////////////////////////////////////////////
unsigned int SDLLayer::getJoystickButtonCount(SDL_Joystick& handle)
{
    const int count = SDL_GetNumJoystickButtons(&handle);

    if (count == -1)
    {
        errMsg("`SDL_GetNumJoystickButtons` failed: {}", SDL_GetError());
        return 0u;
    }

    return static_cast<unsigned int>(count);
}


////////////////////////////////////////////////////////////
unsigned int SDLLayer::getJoystickAxisCount(SDL_Joystick& handle)
{
    const int count = SDL_GetNumJoystickAxes(&handle);

    if (count == -1)
    {
        errMsg("`SDL_GetNumJoystickAxes` failed: {}", SDL_GetError());
        return 0u;
    }

    return static_cast<unsigned int>(count);
}


////////////////////////////////////////////////////////////
unsigned int SDLLayer::getJoystickHatCount(SDL_Joystick& handle)
{
    const int count = SDL_GetNumJoystickHats(&handle);

    if (count == -1)
    {
        errMsg("`SDL_GetNumJoystickHats` failed: {}", SDL_GetError());
        return 0u;
    }

    return static_cast<unsigned int>(count);
}


////////////////////////////////////////////////////////////
[[nodiscard]] const char* SDLLayer::getJoystickName(SDL_Joystick& handle)
{
    const char* name = SDL_GetJoystickName(&handle);

    if (name == nullptr)
    {
        errMsg("`SDL_GetJoystickName` failed: {}", SDL_GetError());
        return nullptr;
    }

    return name;
}


////////////////////////////////////////////////////////////
[[nodiscard]] unsigned int SDLLayer::getJoystickVendor(SDL_Joystick& handle)
{
    const unsigned int vendor = SDL_GetJoystickVendor(&handle);

    if (vendor == 0u)
    {
        errMsg("`SDL_GetJoystickVendor` failed: {}", SDL_GetError());
        return 0u;
    }

    return vendor;
}


////////////////////////////////////////////////////////////
[[nodiscard]] unsigned int SDLLayer::getJoystickProduct(SDL_Joystick& handle)
{
    const unsigned int product = SDL_GetJoystickProduct(&handle);

    if (product == 0u)
    {
        errMsg("`SDL_GetJoystickProduct` failed: {}", SDL_GetError());
        return 0u;
    }

    return product;
}


////////////////////////////////////////////////////////////
[[nodiscard]] bool SDLLayer::areGUIDsEqual(const SDL_GUID& a, const SDL_GUID& b)
{
    return ZB_MEMCMP(&a, &b, sizeof(SDL_GUID)) == 0;
}


////////////////////////////////////////////////////////////
void SDLLayer::setWindowSize(SDL_Window& window, const Vec2u size) const
{
    if (!SDL_SetWindowSize(&window, static_cast<int>(size.x), static_cast<int>(size.y)))
        errMsg("`SDL_SetWindowSize` failed: {}", SDL_GetError());
}


////////////////////////////////////////////////////////////
[[nodiscard]] Vec2u SDLLayer::getWindowSize(SDL_Window& window) const
{
    Vec2i result;

    if (!SDL_GetWindowSize(&window, &result.x, &result.y))
        errMsg("`SDL_GetWindowSize` failed: {}", SDL_GetError());

    return result.toVec2u();
}


////////////////////////////////////////////////////////////
VideoMode SDLLayer::getVideoModeFromSDLDisplayMode(const SDL_DisplayMode& mode) const
{
    const auto* info = getPixelFormatDetails(mode.format);

    if (info == nullptr)
    {
        errMsg("Failed to get pixel format details for display mode");
        return {};
    }

    return {
        .size         = za::Vec2i(mode.w, mode.h).toVec2u(),
        .bitsPerPixel = static_cast<unsigned int>(info->bits_per_pixel),
        .pixelDensity = mode.pixel_density,
        .refreshRate  = mode.refresh_rate,
    };
}


////////////////////////////////////////////////////////////
bool SDLLayer::setGLAttribute(const int attribute, const int value) const
{
    const auto sdlAttribute = static_cast<SDL_GLAttr>(attribute);

    if (SDL_GL_SetAttribute(sdlAttribute, value))
        return true;

    errMsg("Failed to set SDL attribute '{}': {}", getSDLGLAttrName(sdlAttribute), SDL_GetError());
    return false;
}


////////////////////////////////////////////////////////////
bool SDLLayer::applyGLContextSettings(const ContextSettings& settings) const
{
    // Intentionally using bitwise AND to avoid short-circuit evaluation

    bool result = true;

    // Force a basic, flat surface for the window swapchain (Wayland compliant)
    result &= setGLAttribute(SDL_GL_FRAMEBUFFER_SRGB_CAPABLE, 0);
    result &= setGLAttribute(SDL_GL_MULTISAMPLEBUFFERS, 0);
    result &= setGLAttribute(SDL_GL_MULTISAMPLESAMPLES, 0);

    // Set context flags
    result &= setGLAttribute(SDL_GL_DEPTH_SIZE, static_cast<int>(settings.depthBits));
    result &= setGLAttribute(SDL_GL_STENCIL_SIZE, static_cast<int>(settings.stencilBits));
    result &= setGLAttribute(SDL_GL_DOUBLEBUFFER, 1);

    // Set context version
    result &= setGLAttribute(SDL_GL_CONTEXT_MAJOR_VERSION, static_cast<int>(settings.majorVersion));
    result &= setGLAttribute(SDL_GL_CONTEXT_MINOR_VERSION, static_cast<int>(settings.minorVersion));

    // Set context flags
    result &= setGLAttribute(SDL_GL_CONTEXT_PROFILE_MASK,
                             settings.isCore() ? SDL_GL_CONTEXT_PROFILE_CORE : SDL_GL_CONTEXT_PROFILE_COMPATIBILITY);
    result &= setGLAttribute(SDL_GL_CONTEXT_FLAGS, settings.isDebug() ? SDL_GL_CONTEXT_DEBUG_FLAG : 0);

    return result;
}


////////////////////////////////////////////////////////////
const char* SDLLayer::getCurrentVideoDriver() const
{
    const char* driver = SDL_GetCurrentVideoDriver();

    if (driver == nullptr)
        errMsg("`SDL_GetCurrentVideoDriver` failed: {}", SDL_GetError());

    return driver;
}


} // namespace za::priv
