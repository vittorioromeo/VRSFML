...
std::vector<za::Vec2i> touchPositions;
} // namespace
...
{
#if TARGET_IPHONE_SIMULATOR
    zancleMain(*_NSGetArgc(), *_NSGetArgv());
#else
    zancleMain(0, nullptr);
#endif
}
...
- (void)applicationWillResignActive:(UIApplication*)application
{
    ...
    if (self.sfWindow)
        sfWindow->forwardEvent(za::Event::FocusLost{});
}
...
- (void)applicationDidBecomeActive:(UIApplication*)application
{
    ...
    if (self.sfWindow)
        sfWindow->forwardEvent(za::Event::FocusGained{});
}
...
- (void)applicationWillTerminate:(UIApplication*)application
{
    ...
    if (self.sfWindow)
        sfWindow->forwardEvent(za::Event::Closed{});
}
...
{
    za::Vec2u size = self.sfWindow->getSize();
...
- (void)deviceOrientationDidChange:(NSNotification*)notification
{
    if (self.sfWindow)
    {
        ...
        if (UIDeviceOrientationIsValidInterfaceOrientation(orientation))
        {
            ...
            sfWindow->forwardEvent(za::Event::Resized{size});
        }
    }
}
...
////////////////////////////////////////////////////////////
- (za::Vec2i)getTouchPosition:(unsigned int)index
{
...
////////////////////////////////////////////////////////////
- (void)notifyTouchBegin:(unsigned int)index atPosition:(za::Vec2i)position
{
    ...
    if (index >= touchPositions.size())
        touchPositions.resize(index + 1, za::Vec2i(-1, -1));
    touchPositions[index] = position;

    // notify the event to the application window
    if (self.sfWindow)
        sfWindow->forwardEvent(za::Event::TouchBegan{index, position});
}

////////////////////////////////////////////////////////////
- (void)notifyTouchMove:(unsigned int)index atPosition:(za::Vec2i)position
{
    position.x *= static_cast<int>(backingScaleFactor);
    position.y *= static_cast<int>(backingScaleFactor);
    ...
    // notify the event to the application window
    if (self.sfWindow)
        sfWindow->forwardEvent(za::Event::TouchMoved{index, position});
}

////////////////////////////////////////////////////////////
- (void)notifyTouchEnd:(unsigned int)index atPosition:(za::Vec2i)position
{
    // clear the touch position
    if (index < touchPositions.size())
        touchPositions[index] = za::Vec2i(-1, -1);

    // notify the event to the application window
    if (self.sfWindow)
        sfWindow->forwardEvent(za::Event::TouchEnded{index, position * static_cast<int>(backingScaleFactor)});
}
...
{
    if (self.sfWindow)
        sfWindow->forwardEvent(za::Event::TextEntered{character});
}
