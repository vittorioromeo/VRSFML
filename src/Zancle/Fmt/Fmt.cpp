                          ...
                          const SizeT              argCount)
{
    ...
    while (p < end)
    {
        ...
        if (p > litStart)
            ZA_FMT_TRY(sink.append(litStart, static_cast<SizeT>(p - litStart)));
        ...
        if (*p == '{')
        {
            ...
            if (p >= end || *p != '}')
            {
                ZA_FMT_TRY(sink.append("<spec?>", 7u));
                continue;
            }

            ZA_FMT_TRY(dispatchers[argIndex](sink, args[argIndex], spec));
            ++argIndex;
            ++p; // skip '}'
        }
        else // *p == '}'
