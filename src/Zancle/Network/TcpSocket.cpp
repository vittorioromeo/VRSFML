...
[[nodiscard]] zb::String tlsErrorString(int errnum)
{
    char buffer[1024]{};
...
{
#if defined(ZA_SYSTEM_WINDOWS)
    static const auto getErrorString = [](DWORD error) -> zb::String
    {
        ...
        za::Utf<16>::toUtf8(buffer, buffer + srcLen, zb::BackInserter{message});
        LocalFree(buffer);
        return message;
    };
...
struct MbedTlsSharedState
{
    ...
    #define ZA_NUMBER_TO_STRING(x)        ZA_NUMBER_TO_STRING_HELPER(x)
        ...
        static constexpr zb::StringView
            ...
            result != 0)
        {
            ...
            ZB_ASSERT(false && "Failed to seed DRBG");
            zb::abort();
        }
#else
        ...
        if (auto result = psa_crypto_init(); result != PSA_SUCCESS)
        {
            ...
            ZB_ASSERT(false && "Failed to initialize crypto library");
            zb::abort();
        }
#endif
    }
...
TcpSocket::TlsStatus TcpSocket::setupTlsClient(const Utf8String& hostname, const char* certificateChainData)
{
    ...
    return setupTlsClient(hostname, reinterpret_cast<const Byte*>(certificateChainData), ZB_STRLEN(certificateChainData) + 1);
}
...
Socket::Status TcpSocket::send(const void* data, zb::SizeT size, zb::SizeT& sent)
{
    ...
    for (sent = 0; sent < size; sent += static_cast<zb::SizeT>(result))
    {
        if (m_impl->tlsState.hasValue())
        {
            ...
            ZB_ASSERT(m_impl->tlsState->handshakeComplete &&
...
#pragma GCC diagnostic ignored "-Wnull-dereference" // False positive.
    ZB_MEMCPY(m_blockToSendBuffer.data(), &packetSize, sizeof(packetSize));
#pragma GCC diagnostic pop
    if (size > 0)
        ZB_MEMCPY(m_blockToSendBuffer.data() + sizeof(packetSize), data, size);
...
Socket::Status TcpSocket::receive(Packet& packet)
{
    ...
    while (m_pendingPacket.data.size() < packetSize)
    {
        ...
        if (received > 0)
        {
            ...
            ZB_MEMCPY(begin, buffer, received);
        }
    }
