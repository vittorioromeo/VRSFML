...
struct World
{
    ...
    void cleanup()
    {
        entities.erase(std::remove_if(entities.begin(), entities.end(), [](const auto& entity) { return !entity->alive; }),
                       entities.end());
    }
...
{
    static inline auto txSmoke = sf::Texture::loadFromFile("resources/pSmoke.png").value();
    ...
    void draw(sf::RenderTarget& rt) override
    {
        rt.draw(
            ...
            sf::RenderStates{.texture = &txSmoke});
    }
};
...
struct SmokeEmitter : Emitter
{
    void update(World& world, float dt) override
    {
        ...
        world.entities.emplaceBack(p);

        p->position     = position;
        p->velocity     = rng.getVec2f({-0.5f, -0.5f}, {0.5f, 0.5f});
        p->acceleration = {};
        ...
        p->lifeChange     = -0.01f;
        p->scaleChange    = 0.01f;
        p->opacityChange  = -0.01f;
        p->rotationChange = rng.getF(-0.1f, 0.1f);
    }
};
