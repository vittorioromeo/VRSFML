    ...
    while (true)
    {
        ...
        clock.restart();
        {
            ...
            for (auto& emitter : smokeEmitters)
            {
                ...
                auto& p = smokeParticles.emplaceBack();
            ...
            sf::base::vectorSwapAndPopIf(smokeParticles, [](const Particle& p) { return p.life <= 0.f; });
            sf::base::vectorSwapAndPopIf(smokeEmitters, [](const Emitter& e) { return e.life <= 0.f; });
        }
        ...
        clock.restart();
        {
            ...
            ImGui::Text("Number of entities: %zu", smokeEmitters.size() + smokeParticles.size());
