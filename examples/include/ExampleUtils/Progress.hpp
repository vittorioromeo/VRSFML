...
////////////////////////////////////////////////////////////
enum class [[nodiscard]] TickResult : za::U8
{
    Running,
    JustFinished,
    AlreadyFinished
...
struct [[nodiscard]] Progress : ProgressBase
{
    ...
    {
        ZA_ASSERT_AND_ASSUME(delta >= 0.f);

        if (value == 1.f)
            return TickResult::AlreadyFinished;

        ZA_ASSERT_AND_ASSUME(value < 1.f);
        value += delta;
...
struct [[nodiscard]] Transition : ProgressBase
{
    ...
    [[nodiscard]] constexpr TickResult tick(const float delta) noexcept
    {
        ...
        if (reversed)
        {
            ...
            ZA_ASSERT_AND_ASSUME(value > 0.f);
            value -= delta;
...
struct [[nodiscard]] TimedCountdown : Countdown
{
    ...
    {
        ZA_ASSERT_AND_ASSUME(duration > 0.f);
...
////////////////////////////////////////////////////////////
[[nodiscard, gnu::always_inline]] inline float getElapsedOr(const za::Optional<TimedCountdown>& opt, const float fallback) noexcept
{
...
////////////////////////////////////////////////////////////
[[nodiscard, gnu::always_inline]] inline bool isDoneOr(const za::Optional<TimedCountdown>& opt) noexcept
{
