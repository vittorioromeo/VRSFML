...
////////////////////////////////////////////////////////////
constexpr za::Vec2f displaySize{800.f, 400.f};


////////////////////////////////////////////////////////////
// Display for a single axis: vertical bar with a red indicator
////////////////////////////////////////////////////////////
class AxisDisplay : public za::Transformable
{
public:
    AxisDisplay(const za::Font& font, za::Joystick::Axis axis) :
        m_label(font,
                {
        ...
        m_axis(axis)
    {
        const za::Rect2f labelBounds = m_label.getLocalBounds();
    ...
    [[nodiscard]] za::Joystick::Axis getAxis() const
    {
        return m_axis;
    }
...
private:
    static constexpr za::Vec2f barSize{20.f, 50.f};

    za::Text           m_label;
    za::Joystick::Axis m_axis;
    float              m_value{};
};
...
public:
    ...
    [[nodiscard]] za::Joystick::Axis getXAxis() const
    {
        return m_xAxis;
    }

    [[nodiscard]] za::Joystick::Axis getYAxis() const
    {
        return m_yAxis;
    }
    ...
    void draw(za::RenderTarget& target, za::RenderStates states) const
    {
        ...
        target.draw(
            za::CircleShapeData{
                ...
                .fillColor        = m_pressed ? za::Color::Red : za::Color::Transparent,
                .outlineColor     = za::Color::White,
                .outlineThickness = 1.f,
                .radius           = radius,
            },
...
////////////////////////////////////////////////////////////
class ButtonDisplay : public za::Transformable
{
public:
...
public:
                ...
                })
    {
        ...
        constexpr za::Vec2f mainButtonPos{displaySize.x / 2.f + 250.f, displaySize.y / 2.f - 50.f};
        constexpr float     offset = 20.f;

        m_buttons.emplaceBack(font, 3u).position = mainButtonPos + za::Vec2f{0.f, -offset}; // North
        m_buttons.emplaceBack(font, 1u).position = mainButtonPos + za::Vec2f{0.f, offset};  // South
        m_buttons.emplaceBack(font, 2u).position = mainButtonPos + za::Vec2f{offset, 0.f};  // East
        m_buttons.emplaceBack(font, 0u).position = mainButtonPos + za::Vec2f{-offset, 0.f}; // West

        // Triggers
        m_triggers.emplaceBack(font, za::Joystick::Axis::V); // Right trigger
        m_triggers.back().position = {displaySize.x / 2.f + 100.f, displaySize.y / 5.f};
        m_triggers.emplaceBack(font, za::Joystick::Axis::U); // Left trigger
        m_triggers.back().position = {displaySize.x / 2.f - 100.f, displaySize.y / 5.f};

        // Trigger buttons (above the trigger bars)
        m_buttons.emplaceBack(font, 7u).position = m_triggers[0].position + za::Vec2f{0.f, -30.f};
        m_buttons.emplaceBack(font, 6u).position = m_triggers[1].position + za::Vec2f{0.f, -30.f};
        ...
        m_joysticks.emplaceBack(font, za::Joystick::Axis::PovX, za::Joystick::Axis::PovY)
    ...
    void handleEvent(const za::Event& event)
    {
        if (const auto* pressed = event.getIf<za::Event::JoystickButtonPressed>())
            setButtonPressed(pressed->button, true);
        else if (const auto* released = event.getIf<za::Event::JoystickButtonReleased>())
            setButtonPressed(released->button, false);
        else if (const auto* moved = event.getIf<za::Event::JoystickMoved>())
...
private:
    ...
    void setAxis(za::Joystick::Axis axis, float value)
    {
        for (auto& j : m_joysticks)
        {
            if (j.getXAxis() == axis)
...
public:
    ...
    {
        // A row of axes
        constexpr float axisSpacing = displaySize.x / static_cast<float>(za::Joystick::AxisCount + 1u);
        for (unsigned int i = 0u; i < za::Joystick::AxisCount; ++i)
        {
            m_axisDisplays.emplaceBack(font, static_cast<za::Joystick::Axis>(i));
            m_axisDisplays.back().position = {axisSpacing + static_cast<float>(i) * axisSpacing, displaySize.y * 0.25f};
        }

        // Two rows of buttons
        constexpr float buttonSpacing = displaySize.x / (static_cast<float>(za::Joystick::ButtonCount) / 2.f + 1.f);
        for (unsigned int i = 0u; i < za::Joystick::ButtonCount; ++i)
        {
...
int main()
{
    ...
    constexpr za::Vec2f windowSize{displaySize.x, displaySize.y + 200.f};
    ...
    const za::Vec2f selectedSize = indexIndicators[joystickIndex].getGlobalBounds().size * 2.f;

    const auto updateIndicators = [&]
    {
        for (unsigned int i = 0u; i < indexIndicators.size(); ++i)
            indexIndicators[i].setFillColor(za::Joystick::query(i).hasValue() ? za::Color::Green : za::Color::Red);
    };
    updateIndicators();
    ...
    {
        const za::Rect2f bounds = notConnected.getLocalBounds();
    ...
    while (true)
    {
        ...
        while (const za::Optional event = window.pollEvent())
        {
            ...
            if (event->is<za::Event::JoystickConnected>() || event->is<za::Event::JoystickDisconnected>())
            {
                rebuildDisplays();
                updateIndicators();
            }
            ...
            {
                if (keyPressed->code >= za::Keyboard::Key::Num0 && keyPressed->code <= za::Keyboard::Key::Num7)
                {
                    joystickIndex = static_cast<unsigned int>(keyPressed->code) -
                                    static_cast<unsigned int>(za::Keyboard::Key::Num0);
                    rebuildDisplays();
                    updateIndicators();
                }
                else if (keyPressed->code == za::Keyboard::Key::Tab)
                {
        ...
        if (!za::Joystick::query(joystickIndex).hasValue())
