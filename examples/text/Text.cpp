...
struct DemoText
{
    DemoText(const std::filesystem::path& fontFilename, std::u8string_view message) :
        font(std::make_unique<sf::Font>(sf::Font::openFromFile("resources" / fontFilename).value())),
        text(*font, {.string = sf::StringUtfUtils::fromUtf8(message.begin(), message.end()), .characterSize = textSize})
    {
        ...
        text.position    = {(static_cast<float>(windowWidth) - localBounds.size.x) / 2.0f, currentYPosition};
    ...
    void setPosition(const sf::Vec2f& position)
    {
    ...
    void setOrientation(sf::Text::Orientation orientation)
    {
    ...
    {
        target.draw(glyphBox, {.transform = text.getTransform()});
    }

    void drawCursor(sf::RenderTarget& target) const
    {
        target.draw(cursor, {.transform = text.getTransform()});
    }
    ...
    void update()
    {
        ...
        const auto localBounds = text.getLocalBounds();
        boundingBox.position   = text.position + localBounds.position;
        ...
        const auto vertexData = text.getVertices();
        rainbowYPositions.resize(vertexData.size());
    ...
    void updateGlyphBox(int offset)
    {
        ...
        {
            // Horizontal space
            glyphBox.position = {(shapedGlyph.position + shapedGlyph.glyph.bounds.position).x,
        ...
        {
            // Vertical space
            glyphBox.position = {-localBounds.size.x / 2.0f, localBounds.position.y + shapedGlyph.position.y - boxHeight};
            glyphBox.setSize({localBounds.size.x, shapedGlyph.glyph.bounds.size.y});
            glyphBox.setOutlineColor(sf::Color(0, 127, 255));
        }
        else
        {
            glyphBox.position = {shapedGlyph.position + shapedGlyph.glyph.bounds.position};
    ...
    void computeCursorPositions()
    {
        ...
        const auto&         originalGlyphs  = text.getShapedGlyphs();
        ...
        sf::Vec2f           previousClusterPosition;
        sf::Vec2f           previousClusterSize;
        float               previousClusterAdvance{};
        sf::Text::Direction previousClusterDirection{};

        // The cursor interpolation algorithm requires glyphs to be in logical order (sorted by cluster).
        // Create a temporary, sorted copy to ensure correctness without slowing down rendering.
        sf::base::Vector<sf::ShapedGlyph> shapedGlyphs = originalGlyphs;
        std::sort(shapedGlyphs.begin(),
                  shapedGlyphs.end(),
                  [](const auto& a, const auto& b) { return a.cluster < b.cluster; });

        // Set size of cursor depending on text orientation
        if (text.getOrientation() == sf::Text::Orientation::Default)
        {
        ...
        for (const auto& shapedGlyph : shapedGlyphs)
        {
            ...
            sf::Vec2f clusterPosition{shapedGlyph.position.x, shapedGlyph.baseline - ascent};

            // Remember that in RTL text, the "before grapheme position" is to the right of the glyph
            if (shapedGlyph.textDirection == sf::Text::Direction::RightToLeft)
            ...
            if (shapedGlyph.textDirection == sf::Text::Direction::TopToBottom)
            {
                clusterPosition = sf::Vec2f{shapedGlyph.position.x +
            ...
            else if (shapedGlyph.textDirection == sf::Text::Direction::BottomToTop)
            {
                clusterPosition = sf::Vec2f{shapedGlyph.position.x +
                                                (shapedGlyph.glyph.bounds.size.x - globalBounds.size.x) / 2.0f,
                                            shapedGlyph.position.y -
            ...
            while (shapedGlyph.cluster - previousCluster > 1)
            {
                ...
                ++previousCluster;
                cursorPositions.emplaceBack(previousClusterPosition);
            }
        ...
        if (cursorPositions.size() < codepointCount)
        {
            ...
            const auto clusterSubWidth  = (previousClusterDirection == sf::Text::Direction::RightToLeft
            ...
            while (cursorPositions.size() < codepointCount)
            {
                ...
                cursorPositions.emplaceBack(currentPosition);
            }
        }
        ...
        {
            case sf::Text::Direction::LeftToRight:
                cursorPositions.emplaceBack(
                    previousClusterPosition + sf::Vec2f{std::max(previousClusterSize.x, previousClusterAdvance), 0.0f});
                break;
            case sf::Text::Direction::RightToLeft:
                cursorPositions.emplaceBack(
                    previousClusterPosition + sf::Vec2f{-std::max(previousClusterSize.x, previousClusterAdvance), 0.0f});
                break;
            case sf::Text::Direction::TopToBottom:
                cursorPositions.emplaceBack(
                    previousClusterPosition + sf::Vec2f{0.0f, std::max(previousClusterSize.y, previousClusterAdvance)});
                break;
            case sf::Text::Direction::BottomToTop:
                cursorPositions.emplaceBack(
                    previousClusterPosition + sf::Vec2f{0.0f, -std::max(previousClusterSize.y, previousClusterAdvance)});
                break;
            default:
    ...
    void updateCursorPosition(int offset)
    {
        ...
        cursor.position = cursorPositions[(cursorIndex >= cursorPositions.size()) ? cursorPositions.size() - 1 : cursorIndex];
    }

    void handleMousePress(const sf::Vec2f& position)
    {
    ...
    void preProcessGlyph(const sf::ShapedGlyph& shapedGlyph,
                         sf::Text::Style&       style,
                         sf::Color&             fillColor,
        ...
        switch (preProcessingMode)
        {
            ...
            case PreProcessingMode::Color:
                if (colorDirection)
                {
                    ...
                    fillColor = (shapedGlyph.textDirection == sf::Text::Direction::RightToLeft)
                                    ? sf::Color(127, 255, 127)
                                    : sf::Color(255, 255, 127);
                }
                else
    ...
    void cyclePreProcessing()
    {
        ...
        text.setGlyphPreProcessor(
            [this](const sf::ShapedGlyph& shapedGlyph,
                   sf::Text::Style&       style,
                   sf::Color&             fillColor,
                   sf::Color&             outlineColor,
                   float&                 outlineThickness)
    ...
    void updateRainbowText(float delta)
    {
        ...
        for (const auto& shapedGlyph : shapedGlyphs)
        {
            ...
            for (auto index = shapedGlyph.vertexOffset; index < shapedGlyph.vertexOffset + shapedGlyph.vertexCount; ++index)
            {
                ...
                vertices[index].position.y = rainbowYPositions[index - text.getFillVerticesStartIndex()] +
    ...
    sf::base::Vector<sf::Vec2f> cursorPositions;
    sf::RectangleShape          cursor{{}};
    bool                        colorDirection{};
    float                       rainbowColorHue{};
    sf::base::Vector<float>     rainbowYPositions;
    ...
    sf::base::Vector<EffectData> effectData;
...
int main()
{
    ...
    sf::RenderWindow window({.size = {windowWidth, 800u}, .title = "SFML Text", .closable = true});
    window.setVerticalSyncEnabled(true);

    const auto font = sf::Font::openFromFile("resources/tuffy.ttf").value();

    sf::Text instructions(font,
                          {.string        = "F1: Toggle text bounding box\nF2: Toggle glyph bounding box\nF3: Toggle "
                                            "cursor\nF4: Cycle "
                                            "pre-processing (None, Color, Outline, Italicize, Embolden)\nLeft/Right "
                                            "Arrow: Move cursor and glyph bounding box\nMouse Press: Set cursor and glyph "
                                            "bounding box "
                                            "position",
                           .characterSize = 14u});

    instructions.position = {10.0f, 10.0f};

    sf::base::Vector<DemoText> demoTexts;

    // Reserve space to make sure the objects don't get moved
    demoTexts.reserve(64);

    // clang-format off
    demoTexts.emplaceBack("NotoSerif-Regular.ttf",           u8"Hello. How are you? I hope you're having a great day!");
    demoTexts.emplaceBack("NotoSerifArmenian-Regular.ttf",   u8"Բարև։ Ինչպե՞ս ես։ Հուսով եմ՝ լավ օր ես անցկացնում։");
    demoTexts.emplaceBack("NotoSerifGeorgian-Regular.ttf",   u8"გამარჯობა. როგორ ხარ? იმედია, კარგად გაატარე დღე!");
    demoTexts.emplaceBack("NotoNaskhArabic-Regular.ttf",     u8"مرحباً، كيف حالك؟ أتمنى أن يكون يومك سعيداً!");
    demoTexts.emplaceBack("NotoSerif-Regular.ttf",           u8"Hallo. Wie geht es dir? Ich hoffe, du hast einen schönen Tag!");
    demoTexts.emplaceBack("NotoSerifDevanagari-Regular.ttf", u8"नमस्ते। आप कैसे हैं? आशा है आपका दिन अच्छा गुजर रहा होगा!");
    demoTexts.emplaceBack("NotoSerifHebrew-Regular.ttf",     u8"שלום. מה שלומך? מקווה שיש לך יום נהדר!");
    demoTexts.emplaceBack("NotoSerifJP-Regular.subset.ttf",  u8"こんにちは。お元気ですか？素敵な一日をお過ごしください！");
    demoTexts.emplaceBack("NotoSerif-Regular.ttf",           u8"Γεια σας. Τι κάνετε; Ελπίζω να έχετε μια υπέροχη μέρα!");
    demoTexts.emplaceBack("NotoSerifKhmer-Regular.ttf",      u8"ជំរាបសួរ។ សុខសប្បាយជាទេ? សង្ឃឹមថាអ្នកមានថ្ងៃដ៏អស្ចារ្យ!");
    demoTexts.emplaceBack("NotoSerif-Regular.ttf",           u8"Hola. ¿Cómo estás? ¡Que tengas un buen día!");
    demoTexts.emplaceBack("NotoSerifKR-Regular.subset.ttf",  u8"안녕하세요. 잘 지내시죠? 좋은 하루 보내시길 바랍니다!");
    demoTexts.emplaceBack("NotoSerifMyanmar-Regular.ttf",    u8"မင်္ဂလာပါ။ နေကောင်းလား? ကောင်းသောနေ့ကို ပိုင်ဆိုင်နိုင်မည်ဟု မျှော်လင့်ပါသည်။");
    demoTexts.emplaceBack("NotoSerifTamil-Regular.ttf",      u8"வணக்கம். எப்படி இருக்கீங்க? இந்த நாள் உங்களுக்கு ரொம்ப நல்லா அமைய வாழ்த்துக்கள்!");
    demoTexts.emplaceBack("NotoSerif-Regular.ttf",           u8"Здравствуйте. Как поживаете? Надеюсь, у вас отличный день!");
    demoTexts.emplaceBack("NotoSerifTC-Regular.subset.ttf",  u8"你好。你好嗎？祝你今天過得愉快！");
    demoTexts.emplaceBack("NotoSerifThai-Regular.ttf",       u8"สวัสดี สบายดีไหม หวังว่าคุณคงมีวันดีๆ นะ!");
    demoTexts.emplaceBack("NotoSerifTibetan-Regular.ttf",    u8"བདེ་མོ། ཁྱོད་བདེ་མོ? ཁྱེད་རང་ལ་ཉིན་མོ་ཡག་པོ་ཞིག་ཡོང་བའི་རེ་བ་ཡོད།");
    demoTexts.emplaceBack("NotoSerif-Regular.ttf",           u8"Bonjour. Comment allez-vous ? J'espère que vous passez une bonne journée !");
    demoTexts.emplaceBack("NotoSerifEthiopic-Regular.ttf",   u8"ሀሎ። ስላም፧ ጥሩ ቀን እንዳለህ ተስፋ አድርግ!");

    auto& ligatureText            = demoTexts.emplaceBack("NotoSerif-Regular.ttf",          u8"ligatures:\nflowers\nmuffins\nwaffles\nstuff");
    auto& normalText              = demoTexts.emplaceBack("NotoSerif-Regular.ttf",          u8"Normal/Bold/Italic");
    auto& boldText                = demoTexts.emplaceBack("NotoSerif-Regular.ttf",          u8"Normal/Bold/Italic");
    auto& italicText              = demoTexts.emplaceBack("NotoSerif-Regular.ttf",          u8"Normal/Bold/Italic");
    auto& zalgoText               = demoTexts.emplaceBack("NotoSerif-Regular.ttf",          u8"Z̴̢̧͕̱͍͖̱̙̲̳͉̘̐̒͜a̴̲̖̖̖͈͉̪͎̜̽̓̀̀̒͌l̸̰͙̳̗͎͙̼͓̭̥͖̾̀̅̏͐̾͊̃͂̾ͅg̷̡̢͚̗͎̰̦͔̰̬͍̖̩̺̞͕̓̃̇ö̴̜̳̼̘̗͖͍̻͎͓̣̮̝̯͉̞́̄̉̀̄͆̈́͛̐̂̾͐̓̄");
    auto& tabText                 = demoTexts.emplaceBack("NotoSerif-Regular.ttf",          u8"Tab: >\t<");
    auto& rainbowText             = demoTexts.emplaceBack("NotoSerif-Regular.ttf",          u8"Rainbow Text!");
    auto& bidiText                = demoTexts.emplaceBack("NotoNaskhArabic-Regular.ttf",    u8"Aligned multi-line\nنص ثنائي الاتجاه باستخدام مكتبة SFML الإصدار " SFML_VERSION_STRING ".\nالأرقام: 0123456789");
    auto& leftAligned             = demoTexts.emplaceBack("NotoNaskhArabic-Regular.ttf",    u8"Left-Aligned\nText\nنص محاذي");
    auto& centered                = demoTexts.emplaceBack("NotoNaskhArabic-Regular.ttf",    u8"Center-Aligned\nText\nنص محاذي");
    auto& rightAligned            = demoTexts.emplaceBack("NotoNaskhArabic-Regular.ttf",    u8"Right-Aligned\nText\nنص محاذي");
    auto& horizontalStrikethrough = demoTexts.emplaceBack("NotoSerif-Regular.ttf",          u8"Strikethrough");
    auto& horizontalUnderlined    = demoTexts.emplaceBack("NotoSerif-Regular.ttf",          u8"Underlined");
    auto& verticalKorean          = demoTexts.emplaceBack("NotoSerifKR-Regular.subset.ttf", u8"누워서 떡 먹기");
    auto& verticalJapanese        = demoTexts.emplaceBack("NotoSerifJP-Regular.subset.ttf", u8"七転び八起き");
    auto& verticalChinese         = demoTexts.emplaceBack("NotoSerifTC-Regular.subset.ttf", u8"一字值千金");
    auto& topToBottomText         = demoTexts.emplaceBack("NotoSerif-Regular.ttf",          u8"Down Text");
    auto& bottomToTopText         = demoTexts.emplaceBack("NotoSerif-Regular.ttf",          u8"Up Text");
    auto& verticalStrikethrough   = demoTexts.emplaceBack("NotoSerif-Regular.ttf",          u8"Strikethrough");
    auto& verticalUnderlined      = demoTexts.emplaceBack("NotoSerif-Regular.ttf",          u8"Underlined");
    // clang-format on

    ligatureText.setPosition({20.0f, 350.0f});
    normalText.setPosition({20.0f, 620.0f});
    boldText.setPosition({20.0f, 650.0f});
    boldText.setStyle(sf::Text::Style::Bold);
    italicText.setPosition({20.0f, 680.0f});
    italicText.setStyle(sf::Text::Style::Italic);
    zalgoText.setPosition({160.0f, 370.0f});
    tabText.setPosition({20.0f, 300.0f});

    // Rainbow text
    rainbowText.setPosition({300.0f, 10.0f});
    rainbowText.setStyle(sf::Text::Style::Bold);
