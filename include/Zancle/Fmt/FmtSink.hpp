...
/// (usually via `ZA_FMT_TRY`).
///
/// \note `fmt(...)` recursion requires the full `<Zancle/Fmt/Fmt.hpp>`
...
public:
    ...
    {
        ZA_FMT_TRY(ensureRoom(n));

        ZA_MEMCPY(m_pos, data, n);
        m_pos += n;
        return FmtResult::Ok;
    }
    ...
    ////////////////////////////////////////////////////////////
    /// \brief Recursive `fmt`. Defined in `<Zancle/Fmt/Fmt.hpp>` --
