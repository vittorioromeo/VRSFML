...
struct [[nodiscard]] Vec2
{
    ...
    {
        ZB_ASSERT_AND_ASSUME(rhs.x != 0 && "cannot divide by 0 (x coordinate)");
        ZB_ASSERT_AND_ASSUME(rhs.y != 0 && "cannot divide by 0 (y coordinate)");
    ...
    {
        return {ZB_CLAMP(x, mins.x, maxs.x), ZB_CLAMP(y, mins.y, maxs.y)};
    }
    ...
    [[nodiscard, gnu::always_inline, gnu::flatten, gnu::const]] friend constexpr Vec2 operator/(const Vec2 lhs, const T rhs)
    {
        ZB_ASSERT_AND_ASSUME(rhs != 0 && "Vec2::operator/ cannot divide by 0");
...
extern template struct za::Vec2<zb::SizeT>;
