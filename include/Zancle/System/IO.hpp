...
////////////////////////////////////////////////////////////
ZB_DEFINE_ENUM_CLASS_BITWISE_OPS(FileOpenMode);
...
/// which returns `zb::nullOpt` if the file can't be opened; once
...
/// \endcode
///
/// Satisfies `zb::AppendSink` (via `append`) for composition with
/// `zb::fmtTo` / `print` / `printLn`. Errors are **not** visible
...
/// `<ZancleBase/Scn/...>`.
///
/// `InFile` directly satisfies `zb::ScnSource` (via `peek` /
/// `consume`), so it can be handed to `zb::scn<T>` /
/// `zb::scnInto` / friends without an external adapter.
///
////////////////////////////////////////////////////////////
class InFile
{
...
/// `zb::print` / `printLn` / `printErr` / `printErrLn` in
/// `<ZancleBase/Fmt/Fmt.hpp>`. For input parsing -- in-memory or from
/// stdin -- see `<ZancleBase/Scn/Scn.hpp>` and
/// `<ZancleBase/Scn/ScnStdin.hpp>`. `InFile` itself satisfies
/// `zb::ScnSource`, so it can be handed to `scn<T>` / `scnInto`
/// directly.
///
