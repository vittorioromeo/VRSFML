...
struct [[nodiscard]] Vec3
{
    ...
    [[nodiscard, gnu::always_inline, gnu::flatten, gnu::pure]] ZA_SYSTEM_API constexpr Vec3 normalized() const
    {
        ...
        ZB_ASSERT(*this != Vec3<T>() && "cannot normalize a zero vec3");
        return (*this) / length();
    }
    ...
    {
        ZB_ASSERT(rhs.x != 0 && "cannot divide by 0 (x coordinate)");
        ZB_ASSERT(rhs.y != 0 && "cannot divide by 0 (y coordinate)");
        ZB_ASSERT(rhs.z != 0 && "cannot divide by 0 (z coordinate)");
    ...
    [[nodiscard, gnu::always_inline, gnu::flatten, gnu::pure]] friend constexpr Vec3 operator/(const Vec3& lhs, const T rhs)
    {
        ZB_ASSERT(rhs != 0 && "cannot divide by 0");
...
extern template struct za::Vec3<zb::SizeT>;
...
/// \li `za::Vec3<zb::SizeT>` is `za::Vec3uz`
///
