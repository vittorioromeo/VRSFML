...
public:
    ...
    /// `zb::abort()` if the OS refuses to spawn.
    ///
...
private:
    ...
    [[nodiscard]] static priv::ThreadEntry* allocateEntry(FFwd&& callable)
    {
        ...
        auto* const entry = ZB_PLACEMENT_NEW(mem)
        ...
        ZB_PLACEMENT_NEW(reinterpret_cast<char*>(mem) + fOffset) FT(static_cast<FFwd&&>(callable));
        return entry;
    }
