...
#include "ZancleBase/Trait/IsPointer.hpp"
...
{
    using type = ZB_UNDERLYING_TYPE(T);
};
...
{
    if constexpr (ZB_IS_SAME(T, AtomicStorageType<T>))
        return value;
    else
        return ZB_BIT_CAST(AtomicStorageType<T>, value);
}
