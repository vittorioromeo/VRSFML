...
public:
    ...
    [[gnu::always_inline]] TItem* emplace(TItem* const pos, Ts&&... xs)
    {
        ...
        {
            ZB_PLACEMENT_NEW(d + m_size) TItem(static_cast<Ts&&>(xs)...);
            ++m_size;
            return d + index;
        }
    ...
    {
        ZB_ASSERT(m_size + count <= m_capacity);
    ...
    {
        ZB_ASSERT(m_size + sizeof...(items) <= m_capacity);

        TItem* d = data();
        (..., (ZB_PLACEMENT_NEW(d + m_size) TItem(static_cast<TItems&&>(items)), ++m_size));
    }
    ...
    ////////////////////////////////////////////////////////////
    ZB_PRIV_DEFINE_COMMON_VECTOR_OPERATIONS(SmallVector);
};
