...
#include "ZancleBase/Builtin/Ctzll.hpp"
...
public:
    ...
    ///
    /// Computed via `ZB_POPCOUNTLL` per word; the trailing-zero
    ...
    [[nodiscard, gnu::pure]] constexpr SizeT count() const noexcept
    {
        ...
        for (SizeT w = 0u; w < wordCount; ++w)
            total += static_cast<SizeT>(ZB_POPCOUNTLL(m_words[w]));
    ...
    [[nodiscard, gnu::pure]] constexpr SizeT findNextSet(const SizeT from) const noexcept
    {
        ...
        if (word != 0u)
            return w * bitsPerWord + static_cast<SizeT>(ZB_CTZLL(word));

        for (++w; w < wordCount; ++w)
            if (m_words[w] != 0u)
                return w * bitsPerWord + static_cast<SizeT>(ZB_CTZLL(m_words[w]));
    ...
    [[gnu::always_inline]] constexpr void forEachSet(F&& fn) const
    {
        for (SizeT w = 0u; w < wordCount; ++w)
        {
            ...
            while (word != 0u)
            {
                const auto bit = static_cast<SizeT>(ZB_CTZLL(word));
