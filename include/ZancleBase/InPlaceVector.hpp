...
public:
    ...
    enum : bool
    {
        enableTrivialRelocation = ZB_IS_TRIVIALLY_RELOCATABLE(TItem)
    };
    ...
    [[gnu::always_inline]] constexpr TItem* emplace(TItem* const pos, Ts&&... xs)
    {
        ...
        {
            ZB_PLACEMENT_NEW(pos) TItem(static_cast<Ts&&>(xs)...);
            ++m_size;
            return data() + index;
        }
        ...
        ZB_PLACEMENT_NEW(pos) TItem(static_cast<TItem&&>(copy));
    ...
    {
        ZB_ASSERT(m_size < N);
        return *(ZB_PLACEMENT_NEW(data() + m_size++) TItem(static_cast<Ts&&>(xs)...));
    }
    ...
    {
        ZB_ASSERT(m_size + sizeof...(items) <= N);
        (..., ZB_PLACEMENT_NEW(data() + m_size++) TItem(static_cast<TItems&&>(items)));
    }
    ...
    ////////////////////////////////////////////////////////////
    ZB_PRIV_DEFINE_COMMON_VECTOR_OPERATIONS(InPlaceVector);
};
