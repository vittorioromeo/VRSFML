...
///
/// `zb::minipfr` is a small fork of Boost.PFR that does not
...
#include "ZancleBase/Trait/IsUnion.hpp"
...
struct TupleBase<IndexSequence<Is...>, Ts...> : TupleMember<Is, Ts>...
{
    ...
    {
        return static_cast<TupleMember<I, ZB_TYPE_PACK_ELEMENT(I, Ts...)>&>(*this).value;
    }
    ...
    {
        return static_cast<const TupleMember<I, ZB_TYPE_PACK_ELEMENT(I, Ts...)>&>(*this).value;
    }
...
struct Tuple : TupleBase<ZB_INDEX_SEQUENCE_FOR(Ts), Ts...>
{
    ////////////////////////////////////////////////////////////
    using TupleBase<ZB_INDEX_SEQUENCE_FOR(Ts), Ts...>::TupleBase;
};
...
constexpr auto tieAsTuple(T&& obj)
{
    enum : SizeT
    {
        fieldCount = priv::countFields<ZB_REMOVE_CVREF(T)>()
    };
...
{
    return priv::getImpl<I>(tieAsTuple(ZB_FORWARD(obj)));
}


////////////////////////////////////////////////////////////
template <SizeT I, typename T>
using FieldType = ZB_REMOVE_REFERENCE(decltype(getField<I>(declVal<T&>())));


////////////////////////////////////////////////////////////
[[gnu::always_inline]] constexpr void forEachField(auto&& obj, auto&& f)
{
    tieAsTuple(ZB_FORWARD(obj)).forEach(ZB_FORWARD(f));
}
...
{
    static_assert(!ZB_IS_UNION(T), "union reflection is forbidden");
    static_assert(!ZB_IS_ARRAY(T), "impossible to extract name from C-style array");
