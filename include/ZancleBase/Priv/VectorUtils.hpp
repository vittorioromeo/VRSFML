...
namespace zb::priv::VectorUtils
{
////////////////////////////////////////////////////////////
template <typename T>
[[gnu::always_inline]] inline constexpr void moveRange(T* target, T* const srcBegin, T* const srcEnd)
{
    if constexpr (ZB_IS_TRIVIALLY_COPYABLE(T))
    {
        ZB_MEMCPY(target, srcBegin, sizeof(T) * static_cast<SizeT>(srcEnd - srcBegin));
    }
    else
    {
        for (T* p = srcBegin; p != srcEnd; ++p, ++target)
            ZB_PLACEMENT_NEW(target) T(static_cast<T&&>(*p)); // not exception-safe
    }
}
...
{
    if constexpr (ZB_IS_TRIVIALLY_RELOCATABLE(T))
    {
        ZB_MEMCPY(static_cast<void*>(target), srcBegin, sizeof(T) * static_cast<SizeT>(srcEnd - srcBegin));
    }
    else
    {
...
[[gnu::always_inline, gnu::flatten]] inline constexpr T* eraseImpl(T* const end, T* const it)
{
    ...
    if constexpr (ZB_IS_TRIVIALLY_RELOCATABLE(T))
    {
        ...
        if (nextElement < end)
            ZB_MEMMOVE(static_cast<void*>(it), nextElement, static_cast<SizeT>(end - nextElement) * sizeof(T));
...
[[gnu::always_inline, gnu::flatten]] inline constexpr T* eraseRangeImpl(T* const end, T* const first, T* const last)
{
    ...
    if constexpr (ZB_IS_TRIVIALLY_RELOCATABLE(T))
    {
        ...
        if (last < end)
            ZB_MEMMOVE(static_cast<void*>(first), last, static_cast<SizeT>(end - last) * sizeof(T));
...
[[gnu::always_inline, gnu::flatten]] inline constexpr void makeHole(T* const pos, T* const end)
{
    ...
    else
    {
        ...
        ZB_PLACEMENT_NEW(end) T(static_cast<T&&>(*(end - 1)));
...
/// `ZB_PRIV_DEFINE_COMMON_VECTOR_OPERATIONS` macro) live here so
