...
private:
    ...
    [[gnu::cold, gnu::noinline, gnu::flatten]] void reserveImpl(const SizeT targetCapacity)
    {
        ...
        const auto finalNewCapacity = ZB_MAX(targetCapacity, geometricGrowthTarget);

        ZB_ASSERT(finalNewCapacity > capacity()); // Should only be called to grow
    ...
    [[gnu::cold, gnu::noinline, gnu::returns_nonnull]] TItem* growAndEmplace(const SizeT insertIndex, Ts&&... xs)
    {
        ...
        const auto finalNewCapacity      = ZB_MAX(oldSize + 1, geometricGrowthTarget);

        auto* newData = priv::VectorUtils::allocate<TItem>(finalNewCapacity);
        ZB_ASSERT_AND_ASSUME(newData != nullptr);

        // Construct new element first (old buffer still alive, references valid).
        ZB_PLACEMENT_NEW(newData + insertIndex) TItem(static_cast<Ts&&>(xs)...);
...
public:
    ...
    [[gnu::always_inline]] void resize(const SizeT newSize, auto&&... args)
    {
        ...
        if (newSize > oldSize)
        {
            ...
            for (auto* p = m_data + oldSize; p != m_data + newSize; ++p)
                ZB_PLACEMENT_NEW(p) TItem(args...); // intentionally not forwarding
        }
        else
        {
    ...
    [[gnu::always_inline]] TItem* emplace(TItem* const pos, Ts&&... xs)
    {
        ...
        {
            ZB_PLACEMENT_NEW(m_endSize) TItem(static_cast<Ts&&>(xs)...);
            ++m_endSize;
            return m_data + index;
        }
        ...
        ZB_PLACEMENT_NEW(currentPos) TItem(static_cast<TItem&&>(copy));
    ...
    {
        ZB_ASSERT(size() + count <= capacity());
        ZB_ASSERT(m_data != nullptr);
        ZB_ASSERT(m_endSize != nullptr);
    ...
    {
        ZB_ASSERT(m_endSize < m_endCapacity);
        ZB_ASSERT(m_data != nullptr);
        ZB_ASSERT(m_endSize != nullptr);

        auto* const slot = m_endSize++; // Prevents GCC warnings
        ZB_ASSERT_AND_ASSUME(slot != nullptr);

        return *(ZB_PLACEMENT_NEW(slot) TItem(static_cast<Ts&&>(xs)...));
    }
    ...
    {
        ZB_ASSERT(size() + sizeof...(items) <= capacity());
        ZB_ASSERT(m_data != nullptr);
        ZB_ASSERT(m_endSize != nullptr);

        (..., ZB_PLACEMENT_NEW(m_endSize++) TItem(static_cast<TItems&&>(items)));
    }
    ...
    ////////////////////////////////////////////////////////////
    ZB_PRIV_DEFINE_COMMON_VECTOR_OPERATIONS(Vector);
};
