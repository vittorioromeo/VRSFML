...
#include "ZancleBase/Trait/IsTriviallyConstructible.hpp"
...
public:
    ...
    enum : bool
    {
        enableTrivialRelocation = ZB_IS_TRIVIALLY_RELOCATABLE(T)
    };
    ...
    {
        ZB_PLACEMENT_NEW(&m_buffer.obj) T(object);
    }
    ...
    {
        ZB_PLACEMENT_NEW(&m_buffer.obj) T(ZB_MOVE(object));
    }
    ...
    {
        if (m_engaged)
            ZB_PLACEMENT_NEW(&m_buffer.obj) T(rhs.m_buffer.obj);
    }
    ...
    {
        if (m_engaged)
            ZB_PLACEMENT_NEW(&m_buffer.obj) T(ZB_MOVE(rhs.m_buffer.obj));
    }
        ...
        requires(!isTriviallyCopyAssignable<T> && isCopyAssignable<T>)
    {
        ...
        {
            ZB_ASSERT(m_engaged && rhs.m_engaged);
        ...
        requires(!isTriviallyMoveAssignable<T> && isMoveAssignable<T>)
    {
        ...
        {
            ZB_ASSERT(m_engaged && rhs.m_engaged);
            m_buffer.obj = ZB_MOVE(rhs.m_buffer.obj);
        }
    ...
    {
        ZB_PLACEMENT_NEW(&m_buffer.obj) T(ZB_FORWARD(args)...);
    }
    ...
    {
        ZB_PLACEMENT_NEW(&m_buffer.obj) T(ZB_FORWARD(func)());
    }
    ...
    [[gnu::always_inline]] constexpr T& emplace(Args&&... args)
    {
        ...
        return *(ZB_PLACEMENT_NEW(&m_buffer.obj) T(ZB_FORWARD(args)...));
    }
    ...
    [[gnu::always_inline]] constexpr T& emplaceFromFunc(F&& func)
    {
        ...
        return *(ZB_PLACEMENT_NEW(&m_buffer.obj) T(ZB_FORWARD(func)()));
    }
    ...
    [[nodiscard, gnu::always_inline, gnu::pure]] constexpr T&& valueOr(T&& defaultValue) && noexcept
    {
        return ZB_MOVE(m_engaged ? m_buffer.obj : defaultValue);
    }
...
{
    return Optional<ZB_REMOVE_CVREF(Object)>{ZB_FORWARD(object)};
}
...
{
    return Optional<T>{inPlace, ZB_FORWARD(args)...};
}
...
{
    return Optional<decltype(ZB_FORWARD(f)())>{fromFunc, ZB_FORWARD(f)};
}
