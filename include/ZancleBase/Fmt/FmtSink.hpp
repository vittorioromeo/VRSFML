...
/// (usually via `ZB_FMT_TRY`).
///
/// \note `fmt(...)` recursion requires the full `<ZancleBase/Fmt/Fmt.hpp>`
...
public:
    ...
    {
        ZB_FMT_TRY(ensureRoom(n));

        ZB_MEMCPY(m_pos, data, n);
        m_pos += n;
        return FmtResult::Ok;
    }
    ...
    ////////////////////////////////////////////////////////////
    /// \brief Recursive `fmt`. Defined in `<ZancleBase/Fmt/Fmt.hpp>` --
