...
#include "ZancleBase/Fmt/FmtArgDefaultAlign.hpp" // IWYU pragma: export
#include "ZancleBase/Fmt/FmtCString.hpp"         // IWYU pragma: export
#include "ZancleBase/Fmt/FmtResult.hpp"
#include "ZancleBase/Fmt/FmtSink.hpp"    // IWYU pragma: export
#include "ZancleBase/Fmt/FmtSinkRef.hpp" // IWYU pragma: export
#include "ZancleBase/Fmt/FmtSpan.hpp"    // IWYU pragma: export
#include "ZancleBase/Fmt/FmtSpec.hpp"    // IWYU pragma: export
#include "ZancleBase/Fmt/FmtString.hpp"  // IWYU pragma: export
...
// (int, float) -- they live in `<ZancleBase/Fmt/FmtNumeric.hpp>` and pull
...
constexpr void parseFmtSpec(const char*& p, const char* const end, FmtSpec& spec)
{
    ...
    while (p < end && isDigit(*p))
    {
        ...
        if (spec.width >= 65'536)
            ZB_PRIV_FMT_FAIL("Format spec width out of range (>= 65536)");
    ...
    if (p < end && *p == '.')
    {
        ...
        if (p >= end || !isDigit(*p))
            ZB_PRIV_FMT_FAIL("Format spec precision requires at least one digit");
        ...
        while (p < end && isDigit(*p))
        {
            ...
            if (spec.precision > 10)
                ZB_PRIV_FMT_FAIL("Format spec precision out of range (> 10)");
    ...
    if (p < end && *p != '}')
    {
        ...
        if (t != 'd' && t != 'x' && t != 'X' && t != 'o' && t != 'b' && t != 'c' && t != 'f')
            ZB_PRIV_FMT_FAIL("Format spec type tag must be one of d/x/X/o/b/c/f or omitted");
...
[[nodiscard]] consteval FmtStringInfo analyzeFmtString(const FmtSpan fmtStr)
{
    ...
    while (p < end)
    {
        if (*p == '{')
        {
            ...
            // Bare throws here (no `ZB_PRIV_FMT_FAIL`): this
...
struct [[nodiscard]] FmtString
{
    ...
    consteval FmtString(const char* const s) : str{s, ZB_STRLEN(s)}
    {
...
[[nodiscard]] constexpr FmtResult dispatchFmtArg(FmtSink& sink, const T& arg, const FmtSpec& spec) noexcept
{
    ...
    ZB_FMT_TRY(fmtArg(sink, arg, spec)); // ADL
    ...
    ZB_FMT_TRY(sink.ensureRoom(padTotal));
    ...
    if (padLeft != 0u)
    {
        ZB_MEMMOVE(start + padLeft, start, contentSize);
        ZB_MEMSET(start, spec.fill, padLeft);
    }

    if (padRight != 0u)
        ZB_MEMSET(start + padLeft + contentSize, spec.fill, padRight);
...
////////////////////////////////////////////////////////////
[[nodiscard]] ZA_SYSTEM_API FmtResult
...
////////////////////////////////////////////////////////////
ZA_SYSTEM_API void fmtWriteStdout(const char* data, SizeT size);
ZA_SYSTEM_API void fmtWriteStdoutNewline();
ZA_SYSTEM_API void fmtFlushStdout();

ZA_SYSTEM_API void fmtWriteStderr(const char* data, SizeT size);
ZA_SYSTEM_API void fmtWriteStderrNewline();
ZA_SYSTEM_API void fmtFlushStderr();
    ...
    if constexpr (sizeof...(Args) == 0)
    {
        if (fmtStr.isPureLiteral) [[likely]]
        {
            if (bufferSize < fmtStr.str.size)
                return nullptr;
            ZB_MEMCPY(buffer, fmtStr.str.data, fmtStr.str.size);
...
{
    if constexpr (ZB_IS_SAME(decltype(sink.append(data, size)), FmtResult))
