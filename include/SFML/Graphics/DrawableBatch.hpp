...
/// The `flush...` methods are used to signal the GPU that writes are complete.
///
/// This struct is used as a template parameter for `sf::priv::DrawableBatchImpl`.
...
///
/// \see sf::CPUDrawableBatch, sf::PersistentGPUDrawableBatch, sf::RenderTarget::draw(const DrawableBatchImpl&, const RenderStates&)
///
////////////////////////////////////////////////////////////
...
public:
    ...
    /// though indices might be optimized for triangle strips/fans later if supported).
    ...
    /// only supports `sf::PrimitiveType::Triangles`. Other types may lead
    /// to undefined behavior or be ignored.
    ///
    /// The vertices are transformed by the current transform of the `DrawableBatchImpl`.
    ///
    /// \param vertexData Pointer to the array of vertices
    /// \param vertexCount Number of vertices in the array
    /// \param type Primitive type (currently only `sf::PrimitiveType::Triangles` is robustly supported)
    ///
    /// \note Only supports triangle primitives robustly.
    ///
    ////////////////////////////////////////////////////////////
    ...
    /// The sprite's transform is combined with the batch's transform.
    /// Assumes the sprite uses `sf::PrimitiveType::TriangleStrip` internally
    /// which is converted to triangles.
    ///
    ...
    /// and appends it to the batch. The shape's transform is combined
    /// with the batch's transform.
    ///
    ...
    /// and appends it to the batch. The text's transform is combined
    /// with the batch's transform.
    ///
    ...
    /// and appends them to the batch. The batch's current transform is applied.
    ///
    /// \param sdArrow Data defining the arrow shape
    /// \return A `VertexSpan` referring to the added vertices in the batch's (CPU-side) storage.
    ///         The validity of this span depends on the storage type and subsequent operations.
    ///
    ////////////////////////////////////////////////////////////
...
private:
    ...
    ////////////////////////////////////////////////////////////
    /// \brief (Private) Generates and adds vertices for a shape resembling a triangle fan
    ///
    ...
    ////////////////////////////////////////////////////////////
    /// \brief (Private) Adds vertices for a shape's fill to the batch
    ///
    ...
    ////////////////////////////////////////////////////////////
    /// \brief (Private) Adds vertices for a shape's outline to the batch
    ///
...
///
/// This batch type is generally easier to use and manage if data changes
/// very frequently, as it avoids complex GPU synchronization primitives.
/// However, for batches that are modified less often or are very large,
/// `PersistentGPUDrawableBatch` might offer better performance as it
/// can reduce the amount of data transferred to the GPU each frame.
///
/// When a `CPUDrawableBatch` is drawn, its vertex and index data is
/// typically uploaded to the GPU at that point.
///
/// Example:
/// \code
...
///
/// This batch type can offer significant performance advantages for batches
/// that are large or updated somewhat infrequently, as it allows direct
/// modification of GPU data without explicit per-draw uploads. However,
/// it requires careful management of memory synchronization using the
/// `flushVertexWritesToGPU` and `flushIndexWritesToGPU` methods to ensure
...
/// sf::Sprite sprite(texture);
///
/// // Add sprite and get a span (if needed for modification)
/// sf::VertexSpan spriteSpan = batch.add(sprite);
/// // Modify vertices directly if required, then...
/// // batch.flushVertexWritesToGPU(spriteSpan.vertexOffset, spriteSpan.vertexCount);
///
/// // ... add more drawables
///
/// window.draw(batch); // GPU uses already present (and flushed) data
/// \endcode
///
...
///   as it allows direct modification of GPU data, but requires manual
///   flushing of writes.
///
