...
template <typename T>
[[nodiscard, gnu::always_inline]] inline constexpr FmtResult fmtArg(FmtSink& sink, const T& arg, const FmtSpec&) noexcept
    requires requires {
        arg.data();
        arg.size();
    }
{
    return sink.append(arg.data(), static_cast<SizeT>(arg.size()));
}
